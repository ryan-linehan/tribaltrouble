...
public final class Convert {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i < args.length - 1; i++) args_list.add(args[i]);
        ...
        if (op.equals("-mipmaps")) {
            if (images.length != 1)
                throw new IllegalArgumentException(
                        "Can only create mipmaps from one image, not " + images.length);
    ...
    private static Layer loadFile(File file) throws IOException {
        ...
        assert image.getColorType() == PngImage.COLOR_TYPE_RGB
                || image.getColorType() == PngImage.COLOR_TYPE_RGB_ALPHA;
        int channels = image.getColorType() == PngImage.COLOR_TYPE_RGB ? 3 : 4;
        //		final byte[] bytes = getImageData(image);
        int[] ints = new int[width * height];
        try {
            boolean success =
                    new PixelGrabber(image, 0, 0, width, height, ints, 0, width).grabPixels();
            assert success;
        ...
        if (channels == 4) image_layer.a = new Channel(width, height);
    ...
    private static void saveImage(File file, Layer[] images) throws IOException {
        ...
        Image image =
                new Image(images[0].getWidth(), images[0].getHeight(), ByteBuffer.wrap(bytes));
        image.write(file);
    }
    ...
    private static void saveDxtn(File file, Layer[] images) throws IOException {
        ...
        for (int i = 0; i < mipmap_bytes.length; i++) {
            // images[i].saveAsPNG(new File(file.getParentFile(), images[i].getWidth() + "x" +
            // images[i].getHeight() + "-" + file.getName() + ".png"));
        ...
        new DXTImage(
                        (short) images[0].getWidth(),
                        (short) images[0].getHeight(),
                        internal_format,
                        mipmap_bytes)
                .write(file);
    }
    ...
    private static void save(File file, Layer[] images) throws IOException {
        ...
        } else throw new IllegalArgumentException("unknown extension: " + file);
    }
