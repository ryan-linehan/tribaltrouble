...
public final strictfp class FontRenderer {
    ...
    public FontRenderer(
            String src_font_name,
            int font_size,
            int max_image_size,
            int max_chars,
            String font_info_dir,
            String font_tex_dir,
            String font_tex_classpath)
            throws Exception {
        System.out.println(
                "Converting first "
                        + max_chars
        ...
        java.awt.Font src_font =
                java.awt.Font.createFont(java.awt.Font.TRUETYPE_FONT, font_is)
        ...
        float space_width_f =
                (int) StrictMath.ceil(glyph_bounds2.getMinX())
                        - (int) StrictMath.floor(glyph_bounds0.getMaxX());
    ...
    private final int[] calculateImageHeight(
            Font src_font, int image_width, int space_width, char[] chars) {
        BufferedImage image = new BufferedImage(1, 1, BufferedImage.TYPE_4BYTE_ABGR);
        Graphics2D g2d = (Graphics2D) image.getGraphics();
        g2d.setFont(src_font);
        g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (src_font.canDisplay(chars[ch])) {
                ...
                if (baseline_height > max_baseline_height) max_baseline_height = baseline_height;
                ...
                int glyph_width;
                if (i == ' ') glyph_width = space_width;
                else glyph_width = max_x - min_x;
    ...
    private final Channel drawFont(
            Font src_font,
            String font_tex_classpath,
            String font_info_dir,
            String dest_font_name,
            int font_size,
            int max_glyph_height,
            int max_baseline_height,
            int image_width,
            int image_height,
            int space_width,
            char[] chars,
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (src_font.canDisplay(chars[ch])) {
                ...
                // int max_y = (int)StrictMath.ceil(glyph_bounds.getMaxY()) + GLYPH_Y_BORDER;
                int glyph_width;
                if (i == 32) glyph_width = space_width;
                else glyph_width = max_x - min_x;
                if (current_x + glyph_width > image_width) {
        ...
        if (create_xml) {
            ...
            FontInfo font_info =
                    new FontInfo(
                            tex_name, key_map, GLYPH_X_OVERLAP, GLYPH_Y_OVERLAP, max_glyph_height);
            String font_file_name =
                    font_info_dir + File.separator + dest_font_name + "_" + font_size + ".font";
