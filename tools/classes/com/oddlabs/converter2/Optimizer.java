...
public final strictfp class Optimizer {
    ...
    private static final boolean shortsEquals(
            int index1, int index2, int size, short[] array1, short[] array2) {
    ...
    private static final boolean floatsEquals(
            int index1, int index2, int size, float[] array1, float[] array2) {
        for (int i = 0; i < size; i++) {
            if (StrictMath.abs(array1[index1 * size + i] - array2[index2 * size + i])
                    > VERTEX_TRESHOLD) return false;
        }
        return true;
    }

    private static final boolean bytesEquals(
            int index1, int index2, int size, byte[] array1, byte[] array2) {
        for (int i = 0; i < size; i++) {
            if (array1[index1 * size + i] != array2[index2 * size + i]) return false;
        }
        return true;
    }

    private static final void copyFloats(
            int index1, int index2, int size, float[] array1, float[] array2) {
        for (int i = 0; i < size; i++) array2[index2 * size + i] = array1[index1 * size + i];
    }

    private static final void copyObjects(
            int index1, int index2, int size, Object[] array1, Object[] array2) {
        for (int i = 0; i < size; i++) array2[index2 * size + i] = array1[index1 * size + i];
    }

    private static final boolean floatArrayEquals(
            int index1, int index2, float[][] array1, float[][] array2) {
        return floatsEquals(0, 0, array1[index1].length, array1[index1], array2[index2]);
    }

    private static final boolean byteArrayEquals(
            int index1, int index2, byte[][] array1, byte[][] array2) {
        return bytesEquals(0, 0, array1[index1].length, array1[index1], array2[index2]);
    }

    protected static final ModelInfo optimize(
            /*String tex_name, */ int num_vertices,
            float[] vertices,
            float[] normals,
            float[] colors,
            float[] uvs,
        ...
        for (int i = 0; i < num_vertices; i++) {
            ...
            for (j = 0; j < index; j++) {
                boolean equals =
                        floatsEquals(i, j, 3, vertices, r_vertices)
        ...
        for (int i = 0; i < r_uvs2.length; i++) if (r_uvs2[i] != 0f) using_texture_coords2 = true;
        if (!using_texture_coords2) r_uvs2 = null;
    ...
    private static final float[][] stripArray(int length, float[][] array) {
        float[][] copy = new float[length][];
        for (int i = 0; i < length; i++) copy[i] = array[i];
        return copy;
    }

    private static final byte[][] stripArray(int length, byte[][] array) {
    ...
    protected static final SpriteInfo2 convertToSprite(
            String[][] textures, ModelInfo model_info, float[] clear_color) {
        return new SpriteInfo2(
                textures,
                model_info.indices,
                model_info.vertices,
                model_info.normals,
                model_info.texcoords,
                model_info.texcoords2,
                model_info.skin_names,
                model_info.skin_weights,
                clear_color);
    }

    public static final AnimationInfo convertToAnimation(
            /*float[] skeleton_vertices,*/ Bone skeleton,
            Map initial_pose,
            Map[] anim_map,
            int type,
            float wpc) {
        ...
        for (int frame_index = 0; frame_index < num_frames; frame_index++) {
            ...
            normalizeSkeleton(
                    /*new float[]{0, 0, 0}, skeleton_vertices,*/ bones,
                    skeleton,
                    initial_pose,
    ...
    private static final void normalizeSkeleton(
        ...
        Matrix4f.mul(
                absolute_frame_matrix, inverted_absolute_initial_pose_matrix, resulting_matrix);
