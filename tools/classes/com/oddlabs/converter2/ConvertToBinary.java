...
public final strictfp class ConvertToBinary {
    public static final void main(String[] args) {
        if (args.length != 3) throw new RuntimeException("Invalid number of argument");
        ...
        try {
            FileInputStream input_stream =
                    new FileInputStream(src_dir + File.separatorChar + xml_file);
    ...
    private static final void parseGeometry(Node n, String src_dir, String build_dir) {
        if (n.hasChildNodes()) {
    ...
    private static final void parseGroup(Node n, String src_dir, String build_dir) {
        if (n.hasChildNodes()) {
            ...
            for (int i = 0; i < nl.getLength(); i++) {
                ...
                if (child.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (name.equals("sprite")) parseSprite(child, src_dir, new_build_dir);
    ...
    private static final boolean isModified(File src, File dest) {
        return !dest.exists() || dest.lastModified() < src.lastModified();
    }

    private static final ModelObjectInfo[] getModelObjectInfos(Node n, String src_dir) {
        NodeList nl = n.getChildNodes();
        ...
        for (int i = 0; i < nl.getLength(); i++) {
            ...
            if (item.getNodeName().equals("model")) {
                ...
                object_infos.add(
                        new ModelObjectInfo(
                                new File(src_dir, getText(item)), textures, new float[] {r, g, b}));
            }
        }
    ...
    private static final AnimObjectInfo[] getAnimObjectInfos(Node n, String src_dir) {
        NodeList nl = n.getChildNodes();
        List object_infos = new ArrayList();
        for (int i = 0; i < nl.getLength(); i++) {
            Node item = nl.item(i);
            if (item.getNodeName().equals("animation")) {
                float wpc =
                        Float.parseFloat(item.getAttributes().getNamedItem("wpc").getNodeValue());
                assert wpc != 0f;
    ...
    private static final String[][] getTextureInfos(Node n, String src_dir) {
        NodeList nl = n.getChildNodes();
        List object_infos = new ArrayList();
        for (int i = 0; i < nl.getLength(); i++) {
            Node item = nl.item(i);
            if (item.getNodeName().equals("texture")) {
                ...
                String team_name;
                if (team_node != null) team_name = team_node.getNodeValue();
                else team_name = null;
    ...
    private static final void parseSprite(Node n, String src_dir, String build_dir) {
        ...
        if (modified) {
            ...
            if (scale_node != null) scale = Float.parseFloat(scale_node.getNodeValue());
            else scale = 1f;
            ...
            if (skeleton_info != null) {
                Skeleton skeleton =
                        SkeletonLoader.loadSkeleton(getSkeletonObjectInfo(n, src_dir).getFile());
                ...
                for (int i = 0; i < anim_object_infos.length; i++) {
                    ...
                    animations[i] =
                            Optimizer.convertToAnimation(
                                    skeleton.getBoneRoot(),
                                    skeleton.getInitialPose(),
                                    animation_map,
                                    current.getType(),
                                    current.getWPC());
                }
                ...
                animations =
                        new AnimationInfo[] {
                            new AnimationInfo(identity_frame, AnimationInfo.ANIM_LOOP, 1f)
                        };
            ...
            for (int i = 0; i < model_object_infos.length; i++) {
                ...
                ModelInfo model_info =
                        MeshLoader.loadMesh(current.getFile(), name_to_bone_map, scale);
                assert sprite_models[i] == null;
                sprite_models[i] =
                        Optimizer.convertToSprite(
                                current.getTextures(), model_info, current.getClearColor());
            }
            write(new Object[] {sprite_models, animations}, build_file);
        }
    }

    private static final ObjectInfo getSkeletonObjectInfo(Node n, String src_dir) {
        NodeList nl = n.getChildNodes();
        for (int i = 0; i < nl.getLength(); i++) {
            Node item = nl.item(i);
    ...
    private static final void parseLowDetail(Node n, String src_dir, String build_dir) {
        String name = getName(n);
        ...
        if (isModified(object_info.getFile(), build_file)) {
            ...
            LowDetailModel lowdetailmodel =
                    new LowDetailModel(
                            model_info.indices, model_info.vertices, model_info.texcoords);
    ...
    private static final String getName(Node n) {
        return n.getAttributes().getNamedItem("name").getNodeValue();
    }

    private static final int getInt(Node n, String key) {
        String string = n.getAttributes().getNamedItem(key).getNodeValue();
        return Integer.parseInt(string);
    }

    private static final int getTypeFromString(String str) {
        if (str.equals("loop")) return AnimationInfo.ANIM_LOOP;
        else if (str.equals("plain")) return AnimationInfo.ANIM_PLAIN;
        else throw new RuntimeException("Unknown animation type: " + str);
    }
    ...
    private static final void write(Object output, File file) {
