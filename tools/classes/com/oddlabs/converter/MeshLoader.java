...
public final strictfp class MeshLoader {
    ...
    public static final ModelInfo loadMesh(File file, Map name_to_bone_map, float scale) {
        try {
    ...
    private static final ModelInfo createModelInfo(Node node, Map name_to_bone_map, float scale) {
        //		String texture_name =
        // cutTextureName(node.getAttributes().getNamedItem("texture").getNodeValue());
        ...
        for (int i = 0; i < polygon_list.getLength(); i++) {
            ...
            if (polygon.getNodeType() == Node.ELEMENT_NODE
                    && polygon.getNodeName().equals("polygon")) {
                NodeList vertex_list = polygon.getChildNodes();
                int vertex_index = 0;
                for (int j = 0; j < vertex_list.getLength(); j++) {
                    Node vertex = vertex_list.item(j);
                    if (!vertex.getNodeName().equals("vertex")) continue;
                    // vertex data
                    vertices[polygon_index * 9 + vertex_index * 3 + 0] =
                            getAttrFloat(vertex, "x") * scale;
                    vertices[polygon_index * 9 + vertex_index * 3 + 1] =
                            getAttrFloat(vertex, "y") * scale;
                    vertices[polygon_index * 9 + vertex_index * 3 + 2] =
                            getAttrFloat(vertex, "z") * scale;
                    ...
                    for (int skin_index = 0; skin_index < skins.getLength(); skin_index++)
                        if (skins.item(skin_index).getNodeName().equals("skin")) skin_count++;
                    ...
                    for (int k = 0; k < skins.getLength(); k++) {
                        ...
                        if (!skin.getNodeName().equals("skin")) continue;
    ...
    private static final String cutTextureName(String name) {
        if (name.equals("")) return name;
        String result = name.replaceAll("\\\\", "/");
        int last_slash = result.lastIndexOf("/");
        int dot_index = result.lastIndexOf(".");
        if (dot_index != -1) result = result.substring(last_slash + 1, dot_index);
        return result;
    }

    private static final int countPolys(NodeList polygon_list) {
        int counter = 0;
        for (int i = 0; i < polygon_list.getLength(); i++) {
            Node polygon = polygon_list.item(i);
            if (polygon.getNodeType() == Node.ELEMENT_NODE
                    && polygon.getNodeName().equals("polygon")) counter++;
        }
        return counter;
    }
