...
public final strictfp class SVNClassLoader extends URLClassLoader {
    ...
    private final String mapLibraryName(String name) {
        ...
        if (os_name.startsWith("Mac OS X")) return name + ".dylib";
        else if (os_name.startsWith("Windows")) return name + ".dll";
        else if (os_name.startsWith("Linux") || os_name.startsWith("FreeBSD"))
            return "lib" + name + ".so";
        else throw new RuntimeException("Unknown platform " + os_name);
    }
    ...
    protected final String findLibrary(String name) {
        ...
        File[] libs =
                new File[] {
                    new File(lib_root, System.mapLibraryName(name) + ".svn-base"),
                    new File(lib_root, mapLibraryName(name) + ".svn-base")
                };
        for (int i = 0; i < libs.length; i++) {
            if (libs[i].exists() && libs[i].isFile()) return libs[i].getAbsolutePath();
        }
    ...
    public final URL findResource(String name) {
        ...
        else return super.findResource(name);
    }
    ...
    private final URL doFindResource(String name) {
        ...
        while (path_tokenizer.hasMoreTokens()) {
            ...
            } else path = new File(path, path_element);
        }
        return null;
    }
    ...
    protected final Class findClass(String name) throws ClassNotFoundException {
        ...
        if (class_url == null) return super.findClass(name);
        try {
            ...
            while (length > 0) {
                ...
                if (bytes_read == -1) throw new IOException("EOF reached");
                length -= bytes_read;
            }
            return defineClass(name, buffer, 0, buffer.length);
        } catch (IOException e) {
            throw new ClassNotFoundException(
                    "An Exception occurred while trying to load class " + name + ": ", e);
        }
    }
}
