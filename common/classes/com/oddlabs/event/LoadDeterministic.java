...
public final strictfp class LoadDeterministic extends Deterministic {
    ...
    public LoadDeterministic(File logging_file, boolean zipped) {
        try {
            ...
            if (zipped)
                channel =
                        Channels.newChannel(new GZIPInputStream(new FileInputStream(logging_file)));
            else channel = new FileInputStream(logging_file).getChannel();
            buffer.limit(0);
    ...
    private final boolean isDefault(int num_bytes) {
        if (num_defaults > MIN_DEFAULTS) {
            num_defaults--;
            if (isEndOfLog()) System.out.println("***** End of log ***** ");
            return true;
        } else {
    ...
    private final boolean isEndOfLog() {
        if (!buffer.hasRemaining() && num_defaults == MIN_DEFAULTS) return tryFillBuffer();
        else return false;
    }
