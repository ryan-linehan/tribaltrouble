...
public abstract strictfp class Deterministic {
    ...
    public final void checkpoint(long value) {
        ...
        //	assert logged_value == value: "0x" + Long.toHexString(logged_value) + " != 0x" +
    ...
    protected static final int getTraceId() {
        Throwable t = new Throwable();
        StackTraceElement[] stack_trace_elements = t.getStackTrace();
        int hash = 0;
        for (int i = 0; i < stack_trace_elements.length; i++) {
            if (stack_trace_elements[i]
                    .getClassName()
                    .startsWith(Deterministic.class.getPackage().getName())) continue;
