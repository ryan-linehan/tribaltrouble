...
public final strictfp class ByteBufferOutputStream extends OutputStream {
    ...
    public ByteBufferOutputStream(boolean direct) {
        super();
        if (direct) buffer = ByteBuffer.allocateDirect(BUFFER_SIZE);
        else buffer = ByteBuffer.allocate(BUFFER_SIZE);
    }
    ...
    private final void ensureCapacity(int size) {
        if (buffer.remaining() < size) {
            ...
            else new_buffer = ByteBuffer.allocate(buffer.capacity() * 2 + size);
            buffer.flip();
