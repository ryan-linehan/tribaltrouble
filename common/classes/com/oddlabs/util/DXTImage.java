...
public final class DXTImage {
    ...
    private static final ByteBuffer convertToByteBuffer(
            int internal_format, int width, int height, byte[][] mipmaps) {
        int size = 0;
        ...
        for (int i = 0; i < mipmaps.length; i++) buffer.put(mipmaps[i]);
        buffer.flip();
        return buffer;
    }
    ...
    public DXTImage(short width, short height, int internal_format, byte[][] mipmaps) {
        this(
                ...
                height,
                internal_format,
                convertToByteBuffer(internal_format, width, height, mipmaps));
    }
    ...
    private static final int getMipMapSize(int internal_format, int width, int height) {
        int blocksize = 16;
