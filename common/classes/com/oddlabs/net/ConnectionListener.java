...
public final strictfp class ConnectionListener extends AbstractConnectionListener
        implements Handler {
    ...
    private static final SelectionKey createServerSocket(
            NetworkSelector network, InetAddress ip, int port) throws IOException {
    ...
    public ConnectionListener(
        ...
        else network.attachToKey(key, this);
    }
    ...
    public final int getPort() {
        return network.getDeterministic()
                .log(
                        key != null
                                ? ((ServerSocketChannel) key.channel()).socket().getLocalPort()
                                : -1);
    }
    ...
    private final SocketChannel getNextConnection() {
        ...
        if (incoming_connections.size() > 0) notifyIncomingConnection();
        return channel;
    }
    ...
    protected final AbstractConnection doAcceptConnection(ConnectionInterface conn_interface) {
        ...
        if (!network.getDeterministic().isPlayback()) socket_key = getNextConnectionKey();
        else socket_key = null;
    ...
    public final void rejectConnection() {
        try {
            SocketChannel channel = getNextConnection();
            if (!network.getDeterministic().isPlayback()) channel.close();
        } catch (IOException e) {
            System.out.println("Exception: " + e);
            e.printStackTrace();
        }
    ...
    public final void close() {
        if (key != null && key.isValid()) {
            try {
                key.channel().close();
                while (incoming_connections.size() > 0) removeNextChannel().close();
            } catch (IOException e) {
                System.out.println("Exception: " + e);
                e.printStackTrace();
            }
        }
        if (network.getDeterministic().log(key != null)) network.cancelKey(key, this);
    }
