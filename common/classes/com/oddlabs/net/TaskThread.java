...
public final strictfp class TaskThread {
    ...
    static final strictfp class TaskFailed implements TaskResult {
    ...
    static final strictfp class TaskSucceeded implements TaskResult {
    ...
    private void processTasks() {
        ...
        if (notification_action != null) notification_action.run();
    }
            ...
            synchronized (lock) {
                if (!deterministic.log(!finished_tasks.isEmpty())) {
                    ...
                    if (tasks.size() > 0) {
                        ...
                        if (current_task.cancelled && thread != null) thread.interrupt();
                    }
                    return;
                }
                task =
                        (BlockingTask)
                                deterministic.log(
                                        deterministic.isPlayback()
                                                ? null
                                                : finished_tasks.remove(0));
                callable = lookupCallable(task);
            }
            if (!task.cancelled) task.result.deliverResult(callable);
        }
    }
    ...
    public final void close() {
        finished = true;
        if (!deterministic.isPlayback()) thread.interrupt();
    }
}
