...
public final strictfp class Layer {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (brightness >= 0) {
                    putPixelClip(
                            x,
                            y,
                            (r.getPixel(x, y) + brightness * light_r)
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                            (g.getPixel(x, y) + brightness * light_g)
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                            (b.getPixel(x, y) + brightness * light_b)
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow));
                } else {
                    putPixelClip(
                            x,
                            y,
                            (r.getPixel(x, y) + brightness * (1 - ambient_r))
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                            (g.getPixel(x, y) + brightness * (1 - ambient_g))
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                            (b.getPixel(x, y) + brightness * (1 - ambient_b))
                                    * (bumpmap.getPixel(x, y) * shadow + 1 - shadow));
                }
            }
        }
    ...
    public final Layer bumpFast(Channel bumpmap, float lx, float light, float ambient) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                float brightness =
                        lx * (bumpmap.getPixelWrap(x + 1, y) - bumpmap.getPixelWrap(x - 1, y));
                if (brightness >= 0) {
                    brightness = brightness * light;
                    putPixel(
                            x,
                            y,
                            r.getPixel(x, y) + brightness,
    ...
    public final Layer bumpSpecular(
            Channel bumpmap,
            float lx,
            float ly,
            float lz,
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                putPixelClip(
                        x,
                        y,
                        (r.getPixel(x, y) + highlight * light_r)
                                * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                        (g.getPixel(x, y) + highlight * light_g)
                                * (bumpmap.getPixel(x, y) * shadow + 1 - shadow),
                        (b.getPixel(x, y) + highlight * light_b)
                                * (bumpmap.getPixel(x, y) * shadow + 1 - shadow));
            }
        }
        return this;
    ...
    public final Layer place(Layer sprite, int x_offset, int y_offset) {
        ...
        if (a != null && sprite.a != null) a.place(sprite.a, x_offset, y_offset);
        return this;
    }
                ...
                for (int x = 0; x < width; x++) {
                    if (a.getPixel(x, y) == 0) {
                        putPixel(
                                x,
                                y,
                                layer.r.getPixel(x, y),
                                layer.g.getPixel(x, y),
                                layer.b.getPixel(x, y),
                        ...
                        r.putPixel(
                                x,
                                y,
                                r1 - r1 * a2 * alpha_inv + layer.r.getPixel(x, y) * a2 * alpha_inv);
                        g.putPixel(
                                x,
                                y,
                                g1 - g1 * a2 * alpha_inv + layer.g.getPixel(x, y) * a2 * alpha_inv);
                        b.putPixel(
                                x,
                                y,
                                b1 - b1 * a2 * alpha_inv + layer.b.getPixel(x, y) * a2 * alpha_inv);
