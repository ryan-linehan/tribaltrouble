...
public class UpdateRootEntry extends FSRootEntry implements ISVNRootEntry {
    ...
    private final String convertEntryPath(ISVNEntry entry) {
        File converted_path =
                new File(
                        getAdminArea(entry),
                        "text-base" + File.separator + entry.getName() + ".svn-base");
    ...
    public File getWorkingCopyFile(ISVNEntry entry) {
        if (entry.isDirectory()) return new File(getID(), entry.getPath());
        else return new File(getID(), convertEntryPath(entry));
    }
}
