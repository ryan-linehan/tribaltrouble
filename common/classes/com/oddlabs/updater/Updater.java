...
public final strictfp class Updater {
    ...
    private class UpdaterStatusThread implements Runnable {
        ...
        public UpdaterStatusThread() {
            ...
            if (relative_path.exists()) executable = relative_path.getAbsolutePath();
            ...
            String[] cmd_line = {
                ...
                workspace_path.getAbsolutePath(),
                    ...
                    try {
                        ...
                        if (status.getKind() == UpdateStatus.UPDATE_COMPLETE) {
                            ...
                            File data_dir =
                                    new File(
                                            root,
                                            UpdateInfo.DATA_DIR_PREFIX
                                                    + System.currentTimeMillis());
                            if (temp_data.renameTo(data_dir)) addStatus(status);
                            else
                                throw new IOException(
                                        "Could not rename temporary update directory");
    ...
    public final void updateStatus() {
        List status_list =
                (List)
                        deterministic.log(
                                deterministic.isPlayback() ? null : status.getStatusList());
    ...
    public final void cancel() {
        if (!deterministic.isPlayback()) if (!done) status.cancel();
    }
    ...
    public Updater(Deterministic deterministic, UpdateInfo update_info, UpdateHandler handler) {
        ...
        if (!deterministic.isPlayback()) this.status = new UpdaterStatusThread();
