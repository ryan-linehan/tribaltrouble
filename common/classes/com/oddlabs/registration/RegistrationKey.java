...
public final strictfp class RegistrationKey {
    ...
    public static final PublicKey loadPublicKey() throws Exception {
    ...
    public static final boolean verify(PublicKey public_key, SignedObject signed_object)
            throws GeneralSecurityException {
        return signed_object.verify(
                public_key, Signature.getInstance(RegServiceInterface.SIGN_ALGORITHM));
    }

    private static final int computeShifting() {
        return (int) (StrictMath.log(CHAR_TO_WORD.length()) / StrictMath.log(2));
    }

    public static final BigInteger parseBits(String str) throws RegistrationKeyFormatException {
        ...
        for (int i = str.length() - 1; i >= 0; i--) {
            ...
            if (index[0] == -1)
                ...
                throw new RegistrationKeyFormatException(
                        RegistrationKeyFormatException.TYPE_INVALID_CHAR, c);
    ...
    public static final String createString(BigInteger val) throws NumberFormatException {
        String result = "";
        int shifting = computeShifting();
    ...
    private static final byte[] splitToBytes(long l) {
    ...
    public static final String normalize(String key_str) throws RegistrationKeyFormatException {
        return encode(decode(key_str));
    }

    public static final long decode(String key_str) throws RegistrationKeyFormatException {
        key_str = key_str.toUpperCase();
        key_str = key_str.replaceAll("-", "");
        if (key_str.length() != STRIPPED_LENGTH)
            //			throw new RegistrationKeyFormatException("The specified key must be " +
            // STRIPPED_LENGTH + " characters, excluding the dashes.");
            throw new RegistrationKeyFormatException(
                    RegistrationKeyFormatException.TYPE_INVALID_LENGTH, STRIPPED_LENGTH);
        ...
        if (!crc.equals(encoded_crc))
            ...
            throw new RegistrationKeyFormatException(
                    RegistrationKeyFormatException.TYPE_INVALID_KEY);
        return key_code;
    }

    private static final BigInteger computeChecksum(long key) {
    ...
    public static final String encode(long key_code) {
    ...
    private static final void test(long key) {
        String key_encoded = encode(key);
        long key_decoded = decode(key_encoded);
        if (key_decoded != key)
            throw new RuntimeException(
                    "Test failed, key_decoded = " + key_decoded + " != key = " + key);
        // System.out.println("key = " + key + " | key_decoded = " + key_decoded + " | key_encoded =
        // " + key_encoded);
    }
}
