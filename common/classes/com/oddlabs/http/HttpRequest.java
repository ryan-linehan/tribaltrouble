...
public final strictfp class HttpRequest {
    ...
    private static Task spawnPostRequest(
            TaskThread task_thread,
            final URL url,
            final HttpRequestParameters parameters,
            final HttpResponseParser parser,
            final HttpCallback callback) {
        return task_thread.addTask(
                new Callable() {
    ...
    private static Task spawnGetRequest(
            TaskThread task_thread,
            final URL url,
            final HttpResponseParser parser,
            final HttpCallback callback) {
    ...
    private static void copy(InputStream is, OutputStream os) throws IOException {
        ...
        while ((ret = is.read(buf)) > 0) {
            if (os != null) os.write(buf, 0, ret);
        }
    }

    private static HttpResponse runPostRequest(
            URL url, HttpRequestParameters parameters, HttpResponseParser parser)
            throws IOException {
