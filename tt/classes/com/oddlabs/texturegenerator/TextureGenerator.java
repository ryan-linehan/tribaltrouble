...
public final strictfp class TextureGenerator {
    ...
    public TextureGenerator(String dest) {
        ...
        OffscreenRenderer buffer =
                factory.createRenderer(
                        LOW_DETAIL_TEX_SIZE,
                        LOW_DETAIL_TEX_SIZE,
                        new PixelFormat(Globals.VIEW_BIT_DEPTH, 1, 16, 0, 0),
                        false);
        ...
        generateLowDetailTexture(
                buffer,
                new LowDetailModel[] {jungle_lowdetail, palm_lowdetail},
                dest + "/lowdetail_tree");

        OffscreenRenderer viking_buffer =
                factory.createRenderer(
                        LOW_DETAIL_TEX_SIZE,
                        LOW_DETAIL_TEX_SIZE,
                        new PixelFormat(Globals.VIEW_BIT_DEPTH, 1, 16, 0, 0),
        ...
        generateLowDetailTexture(
                viking_buffer,
                new LowDetailModel[] {oak_lowdetail, pine_lowdetail},
                dest + "/viking_lowdetail_tree");
        Display.destroy();
    }

    private final void generateLowDetailTexture(
            OffscreenRenderer buffer, LowDetailModel[] models, String dest) {
        int[] indices = new int[models.length];
        for (int i = 0; i < models.length; i++) {
            indices[i] = i;
        }
        do {
            drawBillboardsToBuffer(
                    models,
                    this,
                    indices,
                    buffer,
                    Globals.COMPRESSED_RGBA_FORMAT,
                    LOWDETAIL_MIPMAP_CUTOFF,
                    0);
            buffer.dumpToFile(dest);
        } while (buffer.isLost());
    }

    private static final void generateBillboardMip(
        ...
        for (int i = 0; i < lowdetail.getIndices().length / 3; i++) {
            BillboardPainter.loadFaceMatrixAndClipPlanes(
                    i, lowdetail.getIndices(), lowdetail.getVertices(), lowdetail.getTexCoords());
            renderer.renderModel(mode, tex_index);
        }
        BillboardPainter.finish();
    }

    private static final void drawBillboardsToBuffer(
            LowDetailModel[] lowdetails,
            TextureGenerator renderer,
            int[] modes,
            OffscreenRenderer buffer,
            int format,
