...
public final strictfp class Wave {
    ...
    public Wave(URL file) throws UnsupportedAudioFileException, IOException {
        ...
        int read = 0;
        int total = 0;
        while ((total < temp_buffer.length)
                && (read = ais.read(temp_buffer, total, temp_buffer.length - total)) != -1) {
            total += read;
        }
    ...
    public static final int getFormat(int channels, int sample_size_in_bits) {
        if (channels == 1 && sample_size_in_bits == 8) return AL10.AL_FORMAT_MONO8;
        else if (channels == 1 && sample_size_in_bits == 16) return AL10.AL_FORMAT_MONO16;
        else if (channels == 2 && sample_size_in_bits == 8) return AL10.AL_FORMAT_STEREO8;
        else if (channels == 2 && sample_size_in_bits == 16) return AL10.AL_FORMAT_STEREO16;
        else throw new RuntimeException("Unsupported wave format");
    }
