...
public strictfp class AbstractAudioPlayer implements Animated {
    ...
    public final void setGain(float gain) {
        if (playing) {
            if (parameters.music) {
                AL10.alSourcef(
                        source.getSource(), AL10.AL_GAIN, gain * Settings.getSettings().music_gain);
            } else {
                AL10.alSourcef(
                        source.getSource(), AL10.AL_GAIN, gain * Settings.getSettings().sound_gain);
            }
        }
    }

    public final void setPos(float x, float y, float z) {
        if (playing) AL10.alSource3f(source.getSource(), AL10.AL_POSITION, x, y, z);
    }
    ...
    public final void registerAmbient() {
        if (source != null) AudioManager.getManager().registerAmbient(source);
    }

    public final void removeAmbient() {
        if (source != null) AudioManager.getManager().removeAmbient(source);
    }
