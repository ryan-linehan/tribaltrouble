...
final strictfp class QueuedAudioPlayer extends AbstractAudioPlayer {
    ...
    QueuedAudioPlayer(AudioSource source, AudioParameters params) {
        ...
        else AL10.alSourcei(source.getSource(), AL10.AL_SOURCE_RELATIVE, AL10.AL_FALSE);
    ...
    public final void refill() { // Run by the Refiller thread
        ...
        while (processed > 0) {
            ...
            //			assert AL10.alIsBuffer(al_buffers.get(al_buffers.position())):
            // al_buffers.get(al_buffers.position()) + " is not a buffer";
            AL10.alSourceUnqueueBuffers(source.getSource(), al_return_buffers);
            //			assert al_return_buffers.get(0) == al_buffers.get(al_buffers.position()):
            // "Unexpected buffer removed: " + al_return_buffers.get(0) + " should be " +
            // al_buffers.get(al_buffers.position());
            ...
            // System.out.println("oldest_buffer = " + oldest_buffer + " | processed = " + processed
            // + " | capacity = " + buffer_streams[oldest_buffer].buffer().capacity() + " | position
            // " + buffer_streams[oldest_buffer].buffer().position() + " | limit " +
            // buffer_streams[oldest_buffer].buffer().limit() + " al_size = " +
            // AL10.alGetBufferi(al_buffers.get(oldest_buffer), AL10.AL_SIZE));
        ...
        if (AL10.alGetSourcei(source.getSource(), AL10.AL_SOURCE_STATE) == AL10.AL_STOPPED)
            AL10.alSourcePlay(source.getSource());
        // System.out.println("		AL10.alGetSourcei(source_index,AL10.AL_SOURCE_STATE) = " +
        //	AL10.alGetSourcei(source.getSource(),AL10.AL_SOURCE_STATE) + " | AL10.AL_STOPPED = " +
        // AL10.AL_STOPPED + " | AL10.AL_PLAYING = " + AL10.AL_PLAYING);
    }
