...
public final strictfp class AudioManager implements AudioImplementation {
    ...
    private static final AudioManager singleton;
    ...
    private static AbstractAudioPlayer doNewAudio(AudioSource source, AudioParameters params) {
        if (params.sound instanceof Audio) return new AudioPlayer(source, params);
        else return new QueuedAudioPlayer(source, params);
    }
    ...
    private AudioSource findSource(AudioParameters params) {
        ...
        for (int i = 0; i < sources.length; i++) {
            ...
            if ((AL10.alGetSourcei(source_index, AL10.AL_SOURCE_STATE) == AL10.AL_STOPPED
                            || AL10.alGetSourcei(source_index, AL10.AL_SOURCE_STATE)
                                    == AL10.AL_INITIAL)
                    && source.getRank() < AudioPlayer.AUDIO_RANK_AMBIENT) {
                if (source.getAudioPlayer() != null) source.getAudioPlayer().stop();
                return source;
            }
            if (worst_rank > source.getRank()) worst_rank = source.getRank();
        }
    ...
    private AudioSource getSource(AudioParameters params) {
        if (!Display.isALCreated()) return null;
    ...
    private AudioSource getSource(CameraState camera_state, AudioParameters params) {
        ...
        else this_dist_squared = getCamDistSquared(camera_state, params.x, params.y, params.z);
