...
public final strictfp class ProgressBar extends GUIObject {
    ...
    private final TextLineRenderer text_renderer =
            new TextLineRenderer(Skin.getSkin().getHeadlineFont());
    ...
    public final void progress(float fraction) {
        int current = 0;
        if (index > 0) current = info[index - 1].getWaypoint();
    ...
    private final void renderText(
        ...
        int offset = 0;
        if (index > 0) offset = info[index - 1].getWaypoint();
        ...
        String string =
                Utils.getBundleString(bundle, "loading", new Object[] {new Integer(percentage)});
        text_renderer.renderCropped(
                0, 0, clip_left, clip_right, clip_bottom, clip_top, new StringBuffer(string));
    }

    protected final void renderGeometry(
            float clip_left, float clip_right, float clip_bottom, float clip_top) {
        if (text_only) renderText(clip_left, clip_right, clip_bottom, clip_top);
        else {
            ProgressBarData data = Skin.getSkin().getProgressBarData();
    ...
    private final void pixelize(ProgressBarInfo[] info, int width) {
        ...
        for (int i = 1; i < info.length - 1; i++) {
            info[i].setWaypoint(
                    info[i - 1].getWaypoint() + (int) ((info[i].getWeight() / sum) * width));
        }
    ...
    private final void renderFill(int y) {
        if (index == 0 && step < left_margin) return;
        ProgressBarData data = Skin.getSkin().getProgressBarData();
        ...
        center[Skin.NORMAL].render(
                left_margin, y, offset - left_margin + (int) step, center[Skin.NORMAL].getHeight());
        if (index == info.length) {
