...
public final strictfp class Slider extends GUIObject {
    ...
    public Slider(int width, int min, int max, int init_value) {
        ...
        assert cardinality > 0 && max >= init_value && init_value >= min
                : "Invalid values. cardinality = "
                        + cardinality
                        + " | max = "
                        + max
                        + " | min = "
        ...
        step =
                (getWidth() - left_offset - right_offset - button.getWidth())
                        / (float) (cardinality - 1);
        setValue(init_value);
        addChild(button);
    ...
    protected final void renderGeometry() {
        ...
        if (isDisabled()) slider.render(0, 0, getWidth(), Skin.DISABLED);
        else slider.render(0, 0, getWidth(), Skin.NORMAL);
    }
    ...
    public void mouseScrolled(int amount) {
        if (!isDisabled()) {
            if (amount < 0) setValue(value - 1 + min);
            else setValue(value + 1 + min);
            button.setFocus();
        }
    }
    ...
    public final void mousePressed(int button, int x, int y) {
        if (!isDisabled()) {
            ...
            if (dx < -step / 2) setValue(value - 1 + min);
            else if (dx > step / 2) setValue(value + 1 + min);
    ...
    public final void setValue(int value) {
        ...
        if (start_value != this.value) valueSetAll(getValue());
    }
    ...
    public final void valueSetAll(int value) {
        ...
        for (int i = 0; i < value_listeners.size(); i++) {
            ...
            if (listener != null) listener.valueSet(value);
        }
    }
