...
public final strictfp class Arrow extends GUIObject {
    ...
    private static final StrictVector4f point = new StrictVector4f();

    private StrictVector4f project3DTo2D(float x, float y, float z) {
        point.set(x, y, z, 1);
        StrictMatrix4f.transform(
                gui_root.getDelegate().getCamera().getState().getProjectionModelView(),
                point,
                point);
        if (point.w < .1f) point.w = .1f;
        float inv_w = 1 / point.w;
        point.set(
                (point.x * inv_w + 1) * .5f * LocalInput.getViewWidth(),
                (point.y * inv_w + 1) * .5f * LocalInput.getViewHeight());
        return point;
    }
    ...
    protected final void renderGeometry() {
        ...
        if (show_always
                || gui_root.getDelegate().getCamera().getState().inNoDetailMode()
                || t < real_t) {
            ...
            GL11.glPushMatrix();
            GL11.glTranslatef(
                    LocalInput.getViewWidth() / 2f + dx * t,
                    LocalInput.getViewHeight() / 2f + dy * t,
                    0f);
            GL11.glRotatef(angle, 0f, 0f, 1f);
            float val =
                    (LocalEventQueue.getQueue().getTime() % SECONDS_PER_FLASH)
                            / (SECONDS_PER_FLASH * .5f);
            if (val > 1f) val = 2f - val;
            val = COLOR_DELTA * val;
            ...
            data.getArrow()
                    .render(
                            -head_x,
                            -head_y,
                            data.getArrow().getWidth(),
                            data.getArrow().getHeight());
            GL11.glEnd();
            GL11.glPopMatrix();
