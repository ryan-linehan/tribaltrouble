...
public final strictfp class Languages {
    ...
    public final boolean hasLanguage(String language) {
        for (int i = 0; i < languages.length; i++)
            if (languages[i][0].equals(language)) return true;
        return false;
    }
    ...
    public final Quad[] getFlags() {
        ...
        if (beta_mode) {
            flags =
                    new Quad[] {
                        Skin.getSkin().getFlagDa(),
                        Skin.getSkin().getFlagEn(),
                        Skin.getSkin().getFlagDe(),
                        Skin.getSkin().getFlagEs(),
                        Skin.getSkin().getFlagIt()
                    };
        } else {
            flags =
                    new Quad[] {
