...
public final strictfp class ColumnButton extends RadioButtonGroupElement {
    ...
    protected void mouseClicked(int button, int x, int y, int clicks) {
        if (isMarked()) sorted_descending = !sorted_descending;
        else sorted_descending = true;
    ...
    protected final void renderGeometry() {
        if (isDisabled()) {
            Skin.getSkin()
                    ...
                    .getButtonUnpressed()
                    .render(0, 0, getWidth(), Skin.DISABLED);
            if (isMarked()) renderMark(Skin.DISABLED);
        } else if (isHovered() && pressed) {
            Skin.getSkin()
                    .getMultiColumnComboBoxData()
                    .getButtonPressed()
                    .render(0, 0, getWidth(), Skin.ACTIVE);
            if (isMarked()) renderMark(Skin.ACTIVE);
        } else if (isActive()) {
            Skin.getSkin()
                    .getMultiColumnComboBoxData()
                    .getButtonUnpressed()
            ...
            Skin.getSkin()
                    .getMultiColumnComboBoxData()
                    .getButtonUnpressed()
                    .render(0, 0, getWidth(), Skin.NORMAL);
            if (isMarked()) renderMark(Skin.NORMAL);
        }
    }

    private final void renderMark(int mode) {
        Quad[] arrow;
        if (sorted_descending) arrow = Skin.getSkin().getMultiColumnComboBoxData().getDescending();
        else arrow = Skin.getSkin().getMultiColumnComboBoxData().getAscending();
