...
public final strictfp class MenuButton extends ButtonObject {
    ...
    private final void scaleHovered() {
        float time =
                (LocalEventQueue.getQueue().getTime() - start_hover_time) % SECONDS_PER_HOVER_CYCLE;
        float cycle_position = time / SECONDS_PER_HOVER_CYCLE;
        float scale =
                1f
                        + HOVER_SCALE_FACTOR
                                * (float) StrictMath.sin(cycle_position * 2 * StrictMath.PI);
    ...
    protected final void renderGeometry(
        ...
        if (isActive()) {
            ...
            GL11.glColor4f(
                    Label.DISABLED_COLOR[0],
                    Label.DISABLED_COLOR[1],
                    Label.DISABLED_COLOR[2],
                    Label.DISABLED_COLOR[3]);
        } else {
