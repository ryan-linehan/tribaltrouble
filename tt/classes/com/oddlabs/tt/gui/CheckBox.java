...
public final strictfp class CheckBox extends GUIObject implements ToolTip {
    ...
    public CheckBox(boolean marked, String text, String tool_tip) {
        ...
        addChild(label);
        label.setPos(
                Skin.getSkin().getCheckBoxMarked()[Skin.NORMAL].getWidth(),
                (Skin.getSkin().getCheckBoxMarked()[Skin.NORMAL].getHeight() - label.getHeight())
                        / 2);
        event_listeners[CHECK_BOX_LISTENER] = new java.util.ArrayList();
        setDim(
                Skin.getSkin().getCheckBoxMarked()[Skin.NORMAL].getWidth() + label.getWidth(),
                Skin.getSkin().getCheckBoxMarked()[Skin.NORMAL].getHeight());
        setCanFocus(true);
    }
    ...
    protected final void renderGeometry() {
        if (isDisabled()) {
            if (marked) Skin.getSkin().getCheckBoxMarked()[Skin.DISABLED].render(0, 0);
            else Skin.getSkin().getCheckBoxUnmarked()[Skin.DISABLED].render(0, 0);
        } else if (isActive()) {
            if (marked) {
                if (pressed && isHovered())
                    Skin.getSkin().getCheckBoxUnmarked()[Skin.ACTIVE].render(0, 0);
                else Skin.getSkin().getCheckBoxMarked()[Skin.ACTIVE].render(0, 0);
            } else {
                if (pressed && isHovered())
                    Skin.getSkin().getCheckBoxMarked()[Skin.ACTIVE].render(0, 0);
                else Skin.getSkin().getCheckBoxUnmarked()[Skin.ACTIVE].render(0, 0);
            }
        } else {
            if (marked) Skin.getSkin().getCheckBoxMarked()[Skin.NORMAL].render(0, 0);
            else Skin.getSkin().getCheckBoxUnmarked()[Skin.NORMAL].render(0, 0);
        }
    }
    ...
    public final void checkedAll(boolean marked) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (listener != null) listener.checked(marked);
        }
    }
