...
public final strictfp class TutorialOverDelegate extends CameraDelegate implements Updatable {
    ...
    public TutorialOverDelegate(
            final WorldViewer viewer,
            TutorialInGameInfo tutorial_info,
            Camera old_camera,
            int tutorial_number) {
        ...
        String tutorial_completed_str =
                Utils.getBundleString(
                        bundle, "tutorial_completed", new Object[] {new Integer(tutorial_number)});
        Label label = new Label(tutorial_completed_str, Skin.getSkin().getHeadlineFont());
        addChild(label);
        label.setPos(
                (getWidth() - label.getWidth()) / 2, (getHeight() - label.getHeight()) * 2 / 3);

        group_buttons = new Group();
        HorizButton button_next =
                new HorizButton(Utils.getBundleString(bundle, "next_tutorial"), 170);
        button_next.addMouseClickListener(new StartTutorialListener(tutorial_number + 1));
        HorizButton button_restart =
                new HorizButton(Utils.getBundleString(bundle, "restart_tutorial"), 170);
        ...
        if (tutorial_number < TutorialForm.NUM_TUTORIALS) group_buttons.addChild(button_next);
    ...
    private final strictfp class StartTutorialListener implements MouseClickListener {
        ...
        public final void mouseClicked(int button, int x, int y, int clicks) {
            if (tutorial_info.setNextTutorial(viewer.getGUIRoot(), tutorial_number)) viewer.close();
        }
    }
}
