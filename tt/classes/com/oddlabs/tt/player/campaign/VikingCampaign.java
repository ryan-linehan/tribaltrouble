...
public final strictfp class VikingCampaign extends Campaign {
        ...
        if (Renderer.isRegistered() || number == 1 || number == 2) {
            Form dialog =
                    new CampaignDialogForm(
                            islands[number].getHeader(),
                            islands[number].getDescription(),
                            null,
                            CampaignDialogForm.ALIGN_IMAGE_LEFT,
                            new IslandListener(network, gui_root, number),
                            true);
            ...
            Form demo_form =
                    new DemoForm(
                            gui_root,
                            Utils.getBundleString(db, "limited_campaign_header"),
                            new GUIImage(512, 256, 0f, 0f, 1f, 1f, "/textures/gui/demo_campaign"),
                            Utils.getBundleString(db, "limited_campaign"));
    ...
    public final void defeated(WorldViewer viewer, String game_over_message) {
        if (getState().getCurrentIsland() == 13) ((VikingIsland13) islands[13]).removeCounter();
