...
public abstract strictfp class GLImage {
    ...
    public static final void updateMipMapsArea(
            GLImage[] mipmaps,
    ...
    public static final void blendMipMapsArea(
            GLImage[] dest_mipmaps,
        ...
        for (int i = 1; i < dest_mipmaps.length; i++) {
            ...
            if (i >= base_fadeout_level) {
                if (i >= mip_map_level)
                    dest_mipmaps[i].drawImageBlended(
                            source_mipmaps[i - mip_map_level],
                            start_x,
                            start_y,
                            start_x,
                            start_y,
    ...
    */
    private static final int averagePixel(
            GLImage last_img,
            int x,
            int y,
            int height_div,
        ...
        col4 = (int) (col4 * inv_num_averaged);
        if (max_alpha) {
            if (col4 >= 128) col4 = 255;
        }
        return (col1 << 24) + (col2 << 16) + (col3 << 8) + col4;
    }

    public final void clearAll(int color) {
        for (int y = 0; y < height; y++) for (int x = 0; x < width; x++) putPixel(x, y, color);
    }

    public final void clear(int x, int y, int width, int height, int color) {
        int yy = y;
        for (; yy < y + height; yy++) {
            for (int xx = x; xx < x + width; xx++) putPixel(yy, xx, color);
        }
    }

    public final void drawImageBlended(
            GLImage img, int dx, int dy, int sx, int sy, int w, int h, float alpha_factor) {
        int spixel;
        int dpixel;
        int sr;
        int sg;
        ...
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                ...
                sa = (spixel >>> 24);
                if (alpha_factor != 1f) {
                    //					System.out.println("sa " + sa + " af " + alpha_factor + " round " +
                    // StrictMath.round(sa * alpha_factor) + " r and " + (StrictMath.round(sa *
                    // alpha_factor) & 0xff));
                ...
                putPixel(
                        x + dx,
                        y + dy,
                        (((sa * sa + da * sa_inverse) / 255) << 24)
                                + (((sr * sa + dr * sa_inverse) / 255) << 16)
                                + ((sg * sa + dg * sa_inverse) / 255 << 8)
                                + ((sb * sa + db * sa_inverse) / 255));
                //				System.out.println("result dp " + Integer.toHexString(pixels[x+dy_loop]) + "
                // sp " + Integer.toHexString(spixel) + " dp " + Integer.toHexString(dpixel) + " sa
                // " + Integer.toHexString(sa) + " sa_inv " + Integer.toHexString(sa_inverse) + " sr
                // " + Integer.toHexString(sr) + " sg " + Integer.toHexString(sg) + " sb " +
                // Integer.toHexString(sb) + " dr " + Integer.toHexString(dr) + " dg " +
                // Integer.toHexString(dg)  + " db " + Integer.toHexString(db) + " da " +
                // Integer.toHexString(da));
            }
        }
    }
    ...
    public final void drawImage(GLImage img, int dx, int dy, int sx, int sy, int w, int h) {
        ...
        for (int i = 0; i < h; i++) {
            ...
            pixels.position(pos);
            // System.out.println("pos = " + pos + " | byte_width = " + byte_width + " |
            // pixels.capacity() = "+ pixels.capacity());
            pixels.limit(pos + byte_width);
            pixels.put(other_pixels);
            //			System.arraycopy(img.getPixelArray(), (sy+i)*img.getWidth() + sx, getPixelArray(),
            // (dy+i)*getWidth() + dx, w);
            pixels.clear();
            other_pixels.clear();
        }
