...
public final strictfp class ProfilesForm extends Form {
    ...
    public ProfilesForm(GUIRoot gui_root, Menu main_menu, SelectGameMenu game_menu) {
        ...
        Label label_headline =
                new Label(
                        Utils.getBundleString(bundle, "profiles_caption"),
        ...
        HorizButton create_profile_button =
                new HorizButton(Utils.getBundleString(bundle, "create_new_profile"), 150);
        addChild(create_profile_button);
        create_profile_button.addMouseClickListener(new CreateProfileListener());

        HorizButton delete_profile_button =
                new HorizButton(Utils.getBundleString(bundle, "delete_profile"), 150);
    ...
    public final void receivedProfiles(Profile[] profiles, String last_nick) {
        ...
        for (int i = 0; i < profiles.length; i++) {
            ...
            if (p.getNick().equalsIgnoreCase(last_nick)) selected_row = row;
        }
        if (selected_row != null) profile_list_box.selectRow(selected_row);
    }
        ...
        public final void mouseClicked(int button, int x, int y, int clicks) {
            ...
            if (nick == null) {
                gui_root.addModalForm(
                        new MessageForm(Utils.getBundleString(bundle, "no_profiles")));
            } else {
                String confirm_str =
                        Utils.getBundleString(bundle, "confirm_delete", new Object[] {nick});
    ...
    public void connectionLost() {
        if (new_profile_form != null) new_profile_form.connectionLost();
        if (confirm_delete_form != null) confirm_delete_form.connectionLost();
        remove();
    }
