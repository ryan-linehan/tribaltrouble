...
public final strictfp class ProgressForm {
    ...
    public static final void setProgressForm(
            NetworkSelector network, GUI gui, LoadCallback callback) {
    ...
    public static final void setProgressForm(
        ...
        current_progress =
                new ProgressForm(
                        ...
                        first_progress,
                        new ProgressBarInfo[] {
                            new ProgressBarInfo("" /*"Loading lanscape resources"*/, 10),
                        ...
                        progress_width,
                        show_tip);
        if (first_progress) load_fadable.fadingDone();
    }

    private ProgressForm(
            NetworkSelector network,
            final GUI gui,
            final Fadable load_fadable,
            ...
            int progress_width,
            boolean show_tip) {
        if (Display.isALCreated()) AudioManager.getManager().stopSources();
        ...
        image =
                new GUIImage(
                        ...
                        0f,
                        0f,
                        (float) image_width / texture_width,
                        (float) image_height / texture_height,
                        texture_name);
        image.setPos(0, 0);
        ...
        if (show_tip) {
            ...
            //			Label tip = new Label(LOADING_TIPS[random.nextInt(LOADING_TIPS.length)],
            // Skin.getSkin().getEditFont());
            tip.setPos(
                    progress_bar.getX() + progress_bar.getWidth() / 2 - tip.getWidth() / 2,
                    progress_bar.getY() - tip.getHeight() - PROGRESSBAR_LOADINGTIP_SPACING);
            delegate.addChild(tip);
        }
    }

    private static final void callback(GUI gui, LoadCallback callback, boolean first_progress) {
        ...
        Fadable start_sources_fadable =
                new Fadable() {
                    public final void fadingDone() {
                        if (Display.isALCreated()) AudioManager.getManager().startSources();
                    }
                };
        ...
        } else gui.newFade(start_sources_fadable, client_root, renderer);
    }
    ...
    public static final void progress(float step) {
