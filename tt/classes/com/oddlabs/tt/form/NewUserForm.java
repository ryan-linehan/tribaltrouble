...
public final strictfp class NewUserForm extends Form {
    ...
    public NewUserForm(NetworkSelector network, GUIRoot gui_root, MainMenu main_menu) {
        ...
        Label label_headline =
                new Label(
                        Utils.getBundleString(bundle, "create_new_user_caption"),
        ...
        // warning
        Label label_one_user =
                new Label(
                        Utils.getBundleString(bundle, "one_user_per_key"),
                        Skin.getSkin().getEditFont());
        ...
        ButtonObject button_create =
                new HorizButton(Utils.getBundleString(bundle, "create_user"), BUTTON_WIDTH_LONG);
    ...
    private final void createUser() {
        ...
        if (!editline_password.getContents().equals(editline_verify.getContents())) {
            ...
            String min_length_err =
                    Utils.getBundleString(
                            bundle,
                            "min_length_error",
                            new Object[] {new Integer(MIN_PASSWORD_LENGTH)});
            ...
            if (!login.isValid())
                gui_root.addModalForm(
                        new MessageForm(Utils.getBundleString(bundle, "invalid_login")));
            else doCreateUser(username, login_details, password, login);
        }
    }

    private final void doCreateUser(
            String username, LoginDetails login_details, String password, Login login) {
        Settings.getSettings().username = username;
        Settings.getSettings().pw_digest = password;
        Form connecting_form =
                new MatchmakingConnectingForm(
                        network, gui_root, this, main_menu, login, login_details);
