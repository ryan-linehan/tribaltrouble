...
public final strictfp class AmbientAudio {
    ...
    public AmbientAudio(AudioImplementation audio_implementation) {
        ...
        ambient_forest_buffer =
                (Audio) Resources.findResource(new AudioFile("/sfx/ambient_forest.ogg"));
        ambient_beach_buffer =
                (Audio) Resources.findResource(new AudioFile("/sfx/ambient_beach.ogg"));
        ambient_wind_buffer =
                (Audio) Resources.findResource(new AudioFile("/sfx/ambient_wind.ogg"));
        ambient_forest =
                audio_implementation.newAudio(
                        new AudioParameters(
                                ambient_forest_buffer,
                                10000f,
                                10000f,
                                10000f,
                                AudioPlayer.AUDIO_RANK_AMBIENT,
                                AudioPlayer.AUDIO_DISTANCE_AMBIENT,
                                AudioPlayer.AUDIO_GAIN_AMBIENT_FOREST,
                                AudioPlayer.AUDIO_RADIUS_AMBIENT_FOREST,
                                1f,
                                true,
                                true,
                                false));
        ambient_beach =
                audio_implementation.newAudio(
                        new AudioParameters(
                                ...
                                AudioPlayer.AUDIO_RANK_AMBIENT,
                                AudioPlayer.AUDIO_DISTANCE_AMBIENT,
                                AudioPlayer.AUDIO_GAIN_AMBIENT_BEACH,
                                AudioPlayer.AUDIO_RADIUS_AMBIENT_BEACH,
                                1f,
                                true,
                                true,
                                false));
        ambient_wind =
                audio_implementation.newAudio(
                        new AudioParameters(
                                ...
                                AudioPlayer.AUDIO_RANK_AMBIENT,
                                AudioPlayer.AUDIO_DISTANCE_AMBIENT,
                                AudioPlayer.AUDIO_GAIN_AMBIENT_WIND,
                                AudioPlayer.AUDIO_RADIUS_AMBIENT_WIND,
                                1f,
                                true,
                                true,
                                false));
            ...
            float dr =
                    2f
                            * (float) Math.sqrt(dx * dx + dy * dy)
                            / meters_per_world; // can use Math here - not gamestate affecting

            // update placement and gain of ambient forest source
            ambient_forest.setPos(
                    0f,
                    0f,
                    heightmap.getNearestHeight(camera.getCurrentX(), camera.getCurrentY())
                            - camera.getCurrentZ()
                            + 8f);
            ambient_forest.setGain(
                    AudioPlayer.AUDIO_GAIN_AMBIENT_FOREST
                            * StrictMath.min(1f, StrictMath.max(0f, 1f - dr + 0.5f)));

            // update placement and gain of ambient beach source
            float factor = 1f;
            if (dr != 0) factor = 1f / dr - 1f;
