...
public abstract strictfp class LinearEmitter extends Emitter {
    ...
    public final void animate(float t) {
        if (started) particle_counter += particles_per_second * t;
        ...
        while (particle_counter >= 1 && (num_particles == -1 || num_particles != 0) && started) {
            int initiated =
                    initParticle(
                            ...
                            growth_rate,
                            energy);
            assert initiated <= num_particles || num_particles == -1
                    : "Too many particles initiated";
            particle_counter -= initiated;
            if (num_particles > 0) num_particles -= initiated;
        }
                ...
                if (particle.getEnergy() > 0f) {
                    ...
                    if (z < landscape_z + particle.getRadiusZ() + offset_z) {
                        ...
                        particle.setVelocity(
                                particle.getVelocityX() * friction,
                                particle.getVelocityY() * friction,
        ...
        reregister();
        if (size == 0 && num_particles == 0) remove();
    }

    protected abstract int initParticle(
            Vector3f position,
            Vector3f velocity,
            Vector3f acceleration,
            Vector4f color,
