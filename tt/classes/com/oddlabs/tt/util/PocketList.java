...
public final strictfp class PocketList {
    ...
    public PocketList(int num_pockets) {
        ...
        for (int i = 0; i < pockets.length; i++) pockets[i] = new ArrayList();
        reset();
    }

    public final void add(int cost, Object obj) {
        if (cost >= pockets.length) cost = pockets.length - 1;
        pockets[cost].add(obj);
        if (cost < min_list_index) min_list_index = cost;
        if (cost > max_list_index) max_list_index = cost;
        size++;
    }
    ...
    public final void clear() {
        for (int i = min_list_index; i <= max_list_index; i++) pockets[i].clear();
        //		check();
        reset();
    }
