...
public final strictfp class StrictGLU {
    private static final StrictMatrix4f final_matrix = new StrictMatrix4f();
    private static final StrictMatrix4f perspective_matrix = new StrictMatrix4f();
    private static final StrictMatrix4f proj_matrix = new StrictMatrix4f();
    private static final StrictVector4f in = new StrictVector4f();
    private static final StrictVector4f out = new StrictVector4f();
    private static final StrictVector3f vector_3f = new StrictVector3f();

    public static void gluPerspective(
            StrictMatrix4f proj, float fovy, float aspect, float zNear, float zFar) {
    ...
    public static boolean gluUnProject(
            ...
            float winy,
            float winz,
            StrictMatrix4f model_matrix,
            StrictMatrix4f proj_matrix,
            int[] viewport,
            float[] obj_pos) {
    ...
    public static void gluPickMatrix(
            StrictMatrix4f proj, float x, float y, float width, float height, int[] viewport) {
        if (width <= 0 || height <= 0) return;

        /* Translate and scale the picked region to the entire window */
        //		GL11.glTranslatef((viewport[2] - 2 * (x - viewport[0])) / width, (viewport[3] - 2 * (y -
        // viewport[1])) / height, 0);
        vector_3f.set(
