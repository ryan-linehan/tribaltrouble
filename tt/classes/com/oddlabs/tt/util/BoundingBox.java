...
public strictfp class BoundingBox {
    ...
    public String toString() {
        return "bmx " + bmin_x + " bmy " + bmin_y + " bmz " + bmin_z + " bxx " + bmax_x + " bxy "
                + bmax_y + " bxz " + bmax_z;
    }

    protected final boolean collides(
            float bmin_x, float bmax_x, float bmin_y, float bmax_y, float bmin_z, float bmax_z) {
        return (bmin_x <= this.bmax_x
                && bmax_x > this.bmin_x
                && bmin_y <= this.bmax_y
                && bmax_y > this.bmin_y
                && bmin_z <= this.bmax_z
    ...
    public final boolean collides(BoundingBox other) {
        return collides(
                other.bmin_x, other.bmax_x, other.bmin_y, other.bmax_y, other.bmin_z, other.bmax_z);
    }

    protected final boolean contains(
            float bmin_x, float bmax_x, float bmin_y, float bmax_y, float bmin_z, float bmax_z) {
        return (bmax_x <= this.bmax_x
                && bmin_x > this.bmin_x
                && bmax_y <= this.bmax_y
                && bmin_y > this.bmin_y
                && bmax_z <= this.bmax_z
    ...
    private final float computeCenteredRadius() {
        ...
        return (float)
                StrictMath.sqrt(
                        radius_squared_x * radius_squared_x
                                + radius_squared_y * radius_squared_y
    ...
    public final float computeCenteredXYRadius() {
        ...
        return (float)
                StrictMath.sqrt(
                        radius_squared_x * radius_squared_x + radius_squared_y * radius_squared_y);
    }
    ...
    public final void setBoundsFromLandscape(
            HeightMap heightmap, int start_x, int start_y, int size_x, int size_y) {
        float corner1 = heightmap.getWrappedHeight(start_x, start_y);
        setBounds(
                start_x * HeightMap.METERS_PER_UNIT_GRID,
                (start_x + size_x) * HeightMap.METERS_PER_UNIT_GRID,
                start_y * HeightMap.METERS_PER_UNIT_GRID,
                (start_y + size_y) * HeightMap.METERS_PER_UNIT_GRID,
                corner1,
                corner1);
