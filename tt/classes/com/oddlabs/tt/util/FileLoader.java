...
public final strictfp class FileLoader implements FileLoaderInterface {
    ...
    public FileLoader(File file, FileLoaderListener listener, int num_bytes) {
        ...
        else newFile(file, LocalEventQueue.getQueue().getDeterministic().log(file.length()));
    }
    ...
    public final void load() {
        if (LocalEventQueue.getQueue()
                .getDeterministic()
                .log(file_channel == null || !file_channel.isOpen())) return;
        buffer.clear();
        IOException exception;
        boolean eof;
        try {
        ...
        if (LocalEventQueue.getQueue().getDeterministic().log(exception != null))
            ...
            data(
                    (byte[]) LocalEventQueue.getQueue().getDeterministic().log(buffer.array()),
