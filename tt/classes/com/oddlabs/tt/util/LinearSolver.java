...
public final strictfp class LinearSolver {
    ...
    public static final void solve(float[][] eq_system_orig, float[] solution) {
        ...
        for (int column = 0; column < eq_system[0].length - 1; column++)
            if (solve(column, pivot_row, eq_system)) pivot_row++;
        // dumpEquation(eq_system);
        reduce(eq_system);
    ...
    private static final float[][] copyEquation(float[][] orig_eq) {
    ...
    private static final boolean checkSolution(float[][] eq_system, float[] solution) {
    ...
    private static final void assignSolutions(float[][] eq_system, float[] solution) {
        int row = 0;
        for (int column = 0; column < eq_system[0].length - 1; column++) {
            ...
            if (!isZero(pivot)) {
                ...
                row++;
                if (row == eq_system.length) break;
            }
        }
    }

    private static final void reduce(float[][] eq_system) {
        for (int row = 1; row < eq_system.length; row++) {
            int column;
            for (column = 0; column < eq_system[0].length; column++) {
                float pivot = eq_system[row][column];
                if (!isZero(pivot)) break;
            }
            if (column == eq_system[0].length) continue;
    ...
    private static final boolean checkEquation(float[][] eq_system) {
        for (int row = 0; row < eq_system.length; row++) {
            int num_non_zero = 0;
            for (int column = 0; column < eq_system[0].length; column++)
                if (!isZero(eq_system[row][column])) num_non_zero++;
            if (num_non_zero == 1 && !isZero(eq_system[row][eq_system[0].length - 1])) return false;
        }
        return true;
    }

    public static final boolean isEqual(float val1, float val2) {
    ...
    private static final void swapRows(int row_index1, int row_index2, float[][] eq_system) {
    ...
    private static final void subtractRow(float scale, float[] row1, float[] row2) {
        for (int i = 0; i < row1.length; i++) row1[i] -= scale * row2[i];
    }

    private static final void scaleRow(float scale, float[] row) {
        for (int column = 0; column < row.length; column++) row[column] *= scale;
    }

    private static final boolean solve(int column, int pivot_row, float[][] eq_system) {
    ...
    */
    public static final void dumpSolution(float[] solution) {
        for (int column = 0; column < solution.length; column++) {
