...
public abstract strictfp class OffscreenRenderer {
    ...
    protected OffscreenRenderer(int width, int height, boolean use_copyteximage) {
        ...
        if (!use_copyteximage) image = new GLIntImage(width, height, GL11.GL_RGBA);
        else image = null;
    }
    ...
    public final void copyToTexture(
            Texture tex, int mip_level, int format, int x0, int y0, int x1, int y1) {
        ...
        if (use_copyteximage) {
            ...
            GL11.glReadPixels(
                    x0, y0, x1, y1, image.getGLFormat(), image.getGLType(), image.getPixels());
            GL11.glTexImage2D(
                    GL11.GL_TEXTURE_2D,
                    mip_level,
                    format,
