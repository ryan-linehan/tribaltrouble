...
public final strictfp class GLUtils {
    ...
    private static final ByteBuffer byte_buf;
    private static final IntBuffer int_buf;
    private static final FloatBuffer plane = BufferUtils.createFloatBuffer(4);
    ...
    public static final GLIntImage loadAsGLImage(String location) {
        Image img = null;
        img = Image.read(com.oddlabs.util.Utils.makeURL(location));
        GLIntImage glimage =
                new GLIntImage(img.getWidth(), img.getHeight(), img.getPixels(), GL11.GL_RGBA);
        return glimage;
    }

    public static final boolean getGLBoolean(int gl_enum) {
        GL11.glGetBooleanv(gl_enum, byte_buf);
        return byte_buf.get(0) == (byte) 1;
    }

    public static final int getGLInteger(int gl_enum) {
    ...
    public static final String takeScreenshot(String filename) {
        if (filename.equals("")) {
            int i = 0;
            File file;
            do {
                ...
                filename =
                        System.getProperty("user.home")
                                + File.separator
                                + filename.substring(0, filename.length() - number.length())
                                + number
                                + ".bmp";
        ...
        GL11.glReadPixels(
                ...
                int_buf.get(2),
                int_buf.get(3),
                pixel_data.getGLFormat(),
                pixel_data.getGLType(),
    ...
    */
    public static void saveTexture(int mipmap_level, String filename) {
        GL11.glGetTexLevelParameteriv(
                GL11.GL_TEXTURE_2D, mipmap_level, GL11.GL_TEXTURE_WIDTH, int_buf);
        int width = int_buf.get(0);
        GL11.glGetTexLevelParameteriv(
                GL11.GL_TEXTURE_2D, mipmap_level, GL11.GL_TEXTURE_HEIGHT, int_buf);
