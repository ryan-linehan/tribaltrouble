...
public final strictfp class CameraState {
    static final float MIN_ANGLE = -(float) StrictMath.PI / 2f; // + 0.01f;
    //  private final static float MAX_ANGLE = (float)StrictMath.PI/2f;// - 0.0001f;
    private static final float MAX_ANGLE = -0.0001f;

    private static final StrictVector3f vector = new StrictVector3f();
    ...
    private float animateValue(
            float delta_t, float current, float target, float smoothness_factor) {
    ...
    public final void updateDirectionAndNormal(
            StrictVector3f f, StrictVector3f u, StrictVector3f s) {
        updateDirectionAndNormal(horiz_angle, vert_angle, f, u, s);
    }

    private static final StrictVector3f tmp = new StrictVector3f();
    private static final StrictVector3f tmp2 = new StrictVector3f();

    private static void updateDirectionAndNormal(
            float hangle, float vangle, StrictVector3f f, StrictVector3f u, StrictVector3f s) {
    ...
    private void doSetView(
            float cx, float cy, float cz, float hangle, float vangle, StrictMatrix4f proj) {
