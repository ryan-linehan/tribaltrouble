...
public final strictfp class MagicUsedTrigger extends IntervalTrigger {
    ...
    public MagicUsedTrigger(
            Unit chieftain, float x, float y, float r, int index, Runnable runnable) {
    ...
    protected final void check() {
        ...
        if (!chieftain.isDead()) {
            if (r * r > dx * dx + dy * dy) {
                if (!blowing
                        && chieftain.getPrimaryController() instanceof MagicController
                        && chieftain.getLastMagicIndex() == index) blowing = true;
