...
public final strictfp class Sky {
    ...
    private static final float[][] SKYDOME_INITCOLOR = {{0.90f, 0.95f, 1f}, {1.50f, 0.90f, 0.65f}};
    private static final float[][] SKYDOME_GRADIENT = {{0.75f, 0.825f, 0.95f}, {0.6f, 0.6f, 0.85f}};
    ...
    private static final float START_ANGLE = -(float) StrictMath.PI / 4f;

    private static final float[][] tex_env_color =
            new float[][] {{0.95f, 0.975f, 1f, 1f}, {1f, 0.95f, 0.8f, 1f}};
    ...
    public Sky(LandscapeRenderer renderer, int terrain_type) {
        this(
                renderer,
                terrain_type,
                (float) (renderer.getHeightMap().getMetersPerWorld() * StrictMath.sqrt(2) / 2),
                6000f,
                20,
                20,
                SKYDOME_OUTER_UTILING,
                SKYDOME_OUTER_VTILING,
                SKYDOME_INNER_UTILING,
                SKYDOME_INNER_VTILING,
                renderer.getHeightMap().getMetersPerWorld() / 2,
                renderer.getHeightMap().getMetersPerWorld() / 2,
                SKYDOME_HEIGHT);
    }
    ...
    private final void setupSky() {
        ...
        GL11.glTexEnvfv(GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_COLOR, color);
        ...
        GLStateStack.switchState(
                GLState.VERTEX_ARRAY
                        | GLState.COLOR_ARRAY
                        | GLState.TEXCOORD0_ARRAY
                        | GLState.TEXCOORD1_ARRAY);
    ...
    public final void render() {
        ...
        for (int i = 0; i < strip_indices.length; i++)
            strip_indices[i].drawRangeElements(
                    GL11.GL_TRIANGLE_STRIP, 0, end, subdiv_axis * 2 + 2, 0);
    ...
    public final void renderSeaBottom() {
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glColor3f(
                Globals.SEA_BOTTOM_COLOR[terrain_type][0],
                Globals.SEA_BOTTOM_COLOR[terrain_type][1],
                Globals.SEA_BOTTOM_COLOR[terrain_type][2]);
        ...
        if (Globals.draw_detail) {
            ...
            GL11.glScalef(
                    Globals.LANDSCAPE_DETAIL_REPEAT_RATE, Globals.LANDSCAPE_DETAIL_REPEAT_RATE, 1);
        }

        bottom_vertices.vertexPointer(3, 0, 0);
        water_indices.drawRangeElements(
                GL11.GL_TRIANGLES, 0, bottom_vertices.capacity() / 3, water_indices.capacity(), 0);
    ...
    private Sky(
        ...
        int num_vertices = 0;
        int num_indices = 0;
        SkyStitchVertex[] previous_vertices =
                makeLandscapeVertices(landscape_renderer.getHeightMap());
        ...
        for (int i = 0; i < NUM_WATER_RINGS; i++) {
            ...
            float ring_radius =
                    inner_radius + (float) StrictMath.pow(radius - inner_radius, radius_factor);
            SkyStitchVertex[] ring_vertices =
                    makeDomeVertices(
                            landscape_renderer.getHeightMap(),
                            i + 1,
                            num_vertices,
                            ring_radius,
                            origin_x,
            ...
            SkyStitchVertex[] stitch_vertices =
                    new SkyStitchVertex[ring_vertices.length + previous_vertices.length];
            System.arraycopy(previous_vertices, 0, stitch_vertices, 0, previous_vertices.length);
            System.arraycopy(
                    ring_vertices,
                    0,
                    stitch_vertices,
                    previous_vertices.length,
                    ring_vertices.length);
    ...
    private static FloatVBO toVBO(SkyStitchVertex[] vertices, float height) {
        ...
        for (int i = 0; i < vertices.length; i++) {
            ...
            // System.out.println("z = " + z + " | vertex.getSide() = " + vertex.getSide() + " |
            // NUM_WATER_RIGS = " + NUM_WATER_RINGS + " | height = " + height + " " + (z ==
            // height));
    ...
    private final void makeSkyVertices(
            float radius,
            float outer_utile,
            float outer_vtile,
            float inner_utile,
        ...
        float[] skydome_default_color =
                new float[] {
                    (float)
                            StrictMath.pow(
                                    SKYDOME_GRADIENT[terrain_type][0], SKYDOME_DEFAULT_COLOR),
                    (float)
                            StrictMath.pow(
                                    SKYDOME_GRADIENT[terrain_type][1], SKYDOME_DEFAULT_COLOR),
        ...
        for (int i = 1; i < SKYDOME_GRADIENT_LENGTH; i++) {
            ...
            skydome_gradient[i] =
                    new float[] {
                        alpha * skydome_default_color[0]
                                + (1f - alpha)
                                        * skydome_gradient[i - 1][0]
                                        * SKYDOME_GRADIENT[terrain_type][0],
                        alpha * skydome_default_color[1]
                                + (1f - alpha)
                                        * skydome_gradient[i - 1][1]
                                        * SKYDOME_GRADIENT[terrain_type][1],
                        alpha * skydome_default_color[2]
                                + (1f - alpha)
                                        * skydome_gradient[i - 1][2]
                                        * SKYDOME_GRADIENT[terrain_type][2]
                    };
        }
        ...
        for (int i = 0; i < subdiv_height - 1; i++) {
            ...
            if (java.lang.StrictMath.abs(z) < 250f) height_coeff = dome_height / 250f;
            else height_coeff = dome_height / z;

            for (int j = 0; j < subdiv_axis; j++) {
                x =
                        (float)
                                        java.lang.StrictMath.cos(
                                                START_ANGLE + a_angle_inc * j + offset_angle * i)
                                * r;
                y =
                        (float)
                ...
                else putArray(skydome_default_color, i * subdiv_axis + j, colors);

                putArray(
                        new float[] {x + origin_x, y + origin_y, z + origin_z},
                        i * subdiv_axis + j,
                        vertices);
                putArray(
                        new float[] {
                            x * height_coeff / (radius * outer_utile) + 0.5f,
                            y * height_coeff / (radius * outer_vtile) + 0.5f
                        },
                        i * subdiv_axis + j,
                        tex0);
                putArray(
                        new float[] {
                            x * height_coeff / (radius * inner_utile) + 0.5f,
                            y * height_coeff / (radius * inner_vtile) + 0.5f
                        },
                        i * subdiv_axis + j,
                        tex1);
            }
        ...
        else putArray(skydome_default_color, last_index, colors);
    ...
    private final void putArray(float[] src, int offset, float[] dest) {
        for (int i = 0; i < src.length; i++) dest[i + (offset * src.length)] = src[i];
    }
    ...
    private final SkyStitchVertex[] makeDomeVertices(
            HeightMap heightmap,
            int ring_id,
            int index_offset,
            float radius,
        ...
        for (int i = 0; i < subdiv_axis; i++) {
            ...
            result[i] =
                    new SkyStitchVertex(
                            ...
                            index,
                            ring_id,
                            (float) java.lang.StrictMath.cos(START_ANGLE + a_angle_inc * i) * radius
                                    + origin_x,
                            (float) java.lang.StrictMath.sin(START_ANGLE + a_angle_inc * i) * radius
                                    + origin_y);
        }
        return result;
    }
    ...
    private final SkyStitchVertex[] makeLandscapeVertices(HeightMap heightmap) {
        ...
        for (int i = 0; i < heightmap.getPatchesPerWorld(); i++) {
            int index = i;
            result[index] =
                    new SkyStitchVertex(heightmap, index, 0, 0, heightmap.getMetersPerPatch() * i);
