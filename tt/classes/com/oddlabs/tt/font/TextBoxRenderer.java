...
public final strictfp class TextBoxRenderer {
    ...
    private final void render(
            int x, int y, int offset_y, CharSequence text, int index, boolean render) {
        int clip_left = x;
        int clip_bottom = y;
    ...
    private final int getWordWidth(int render_pos, CharSequence text) {
        ...
        while (render_pos < text.length()) {
            ...
            if (quad != null) width += quad.getWidth() - font.getXBorder();
            render_pos++;
        }
        return width;
    }

    private int renderWord(
            int render_pos,
            int word_width,
            int clip_left,
            int clip_bottom,
        ...
        while (word_width > 0) {
            ...
            if (quad != null) {
                ...
                if (render && in_view)
                    quad.renderClipped(
                            render_x, render_y, clip_left, clip_right, clip_bottom, clip_top);
            ...
            if (key == '\n') {
                ...
                if (quad != null) {
                    ...
                    if (render_x - font.getXBorder() / 2 + width + next_word_width < clip_right) {
                        if (render) quad.render(render_x, render_y);
