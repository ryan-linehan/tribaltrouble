...
public final strictfp class GeneratorPoison extends TextureGenerator {
    ...
    public final Texture[] generate() {
        ...
        */
        ...
        Channel poison_alpha =
                noise.copy()
                        .channelMultiply(blob.copy().dynamicRange(.25f, 1f))
                        .channelSubtract(blob.copy().invert().brightness(.25f));

        Layer poison =
                new Layer(
                        noise.copy().rotate(90).dynamicRange(.5f, 1f),
                        new Channel(TEXTURE_SIZE, TEXTURE_SIZE).fill(1f),
                        new Channel(TEXTURE_SIZE, TEXTURE_SIZE),
                        poison_alpha.brightness(.75f));
        GLIntImage poison_img = new GLIntImage(poison);
        if (Landscape.DEBUG) poison_img.saveAsPNG("generator_poison");
        Texture[] textures = new Texture[1];
        textures[0] =
                new Texture(
                        poison_img.createMipMaps(),
