            ...
            for (int x_block = 0; x_block < (1 << base_freq); x_block++) {
                for (int y_block = 0; y_block < (1 << base_freq); y_block++) {
                    ...
                    channel.putPixel(
                            x,
                            y,
                            base.getPixel(x_block, y_block) + random.nextFloat() * amp - amp_half);
                }
            }
        }
            ...
            if (i < 2) {
                for (x_block = 0, x = 0; x_block < (1 << i); x_block++) {
                    for (y_block = 0, y = 0; y_block < (1 << i); y_block++) {
                        ...
                        v6 =
                                channel.getPixel(
                                        ((x - block_size_half) + size) % size,
                                        (y + block_size_half) % size);
                        v7 =
                                channel.getPixel(
                                        (x + block_size_half) % size,
                                        ((y - block_size_half) + size) % size);
