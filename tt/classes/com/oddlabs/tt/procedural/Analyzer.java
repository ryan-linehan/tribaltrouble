...
public final strictfp class Analyzer {
    ...
    public static final void analyze(Channel height, String name) {
        ...
        System.out.println(
                "Accessible area: "
                        + (100f * access.count(1f) / (access.width * access.height))
                        + "%");
        Channel access_conn = access.copy().largestConnected(1f);
        access_conn.toLayer().saveAsPNG(name + "_access_conn");
        System.out.println(
                "Largest connected accessible area: "
                        + (100f * access_conn.count(1f) / (access_conn.width * access_conn.height))
                        + "%");
        float access_avrconn = access.averageConnected(1f);
        System.out.println(
                "Average connected accessible area: "
                        + (100f * access_avrconn / (access.width * access.height))
                        + "%");
        ...
        System.out.println("done");
        System.out.println(
                "Flat area: " + (100f * flat.count(1f) / (flat.width * flat.height)) + "%");
        Channel flat_conn = flat.copy().largestConnected(1f);
        flat_conn.toLayer().saveAsPNG(name + "_flatness_conn");
        System.out.println(
                "Largest connected flat area: "
                        + (100f * flat_conn.count(1f) / (flat_conn.width * flat_conn.height))
                        + "%");
        float flat_avrconn = flat.averageConnected(1f);
        System.out.println(
                "Average connected flat area: "
                        + (100f * flat_avrconn / (flat.width * flat.height))
                        + "%");
        ...
        histogram(ffts[0], height.width)
                .flipV()
                .toLayer()
                .saveAsPNG(name + "_fft_magnitude_log_histogram");
    ...
    public static final Channel histogram(Channel channel, int size) {
    ...
    public static final void score(Channel channel, String name) {
    ...
    public static final float score(Channel channel) {
        float average = average(channel);
        float variance = variance(channel);
        float deviation = standardDeviation(variance);
    ...
    public static final void statistics(Channel channel, String name) {
        float average = average(channel);
        float variance = variance(channel);
        float deviation = standardDeviation(variance);
    ...
    public static final float average(Channel channel) {
        float sum = 0;
    ...
    public static final float variance(Channel channel) {
        float average = average(channel);
        float sum = 0;
        for (int x = 0; x < channel.width; x++) {
            for (int y = 0; y < channel.height; y++) {
    ...
    public static final float deviation(Channel channel) {
        return (float) Math.sqrt(variance(channel));
    }

    public static final float standardDeviation(float variance) {
        return (float) Math.sqrt(variance);
    }

    public static final Channel connectedness(Channel height, int steps) {
        ...
        for (int i = 0; i < steps; i++) {
            // channel.channelBrightest(slope.copy().threshold(0f,
            // 16f*i/(height.width*steps)).largestConnected(1f).multiply((float)(steps - i)/steps));
            channel.channelAdd(
                    slope.copy()
                            .threshold(0f, 16f * i / (height.width * steps))
                            .largestConnected(1f)
                            .multiply(1f / steps));
            System.out.print(".");
        }
    ...
    public static final float squareScore(Channel height, float threshold, int square_size) {
