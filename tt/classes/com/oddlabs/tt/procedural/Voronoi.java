...
public final strictfp class Voronoi {
    ...
    public Voronoi(
            int size, int x_domains, int y_domains, int checkradius, float randomness, long seed) {
    ...
    public Voronoi(
        ...
        checkradius =
                StrictMath.min(
                        StrictMath.max(1, checkradius), StrictMath.max(x_domains, y_domains));
        ...
        for (int j = 0; j < y_domains; j++) {
            for (int i = 0; i < x_domains; i++) {
                domains[i][j][X] =
                        (1 - randomness) * ((i + .5f) / x_domains)
                                + randomness * ((i + random.nextFloat()) / x_domains);
                domains[i][j][Y] =
                        (1 - randomness) * ((j + .5f) / y_domains)
                                + randomness * ((j + random.nextFloat()) / y_domains);
            ...
            for (int x = 0; x < size; x++) {
                ...
                for (int l = -checkradius; l <= checkradius; l++) {
                    ...
                    for (int k = -checkradius; k <= checkradius; k++) {
                        ...
                        // calculate distance to current hit point taking wrap-around into
                        // consideration
        ...
        for (int y = 0; y < size; y++) {
            for (int x = 0; x < size; x++) {
                channel.putPixel(
                        x,
                        y,
                        c1 * dist1.getPixel(x, y)
                                + c2 * dist2.getPixel(x, y)
                                + c3 * dist3.getPixel(x, y));
            }
        }
