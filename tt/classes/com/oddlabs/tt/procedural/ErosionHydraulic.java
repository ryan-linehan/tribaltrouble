...
public final strictfp class ErosionHydraulic {
    ...
    public static final Channel erodeReference(Channel channel, int iterations) {
        height = channel;
    ...
    private static final void transport() {
        ...
        for (int y = 1; y < height.height - 1; y++) {
            for (int x = 1; x < height.width - 1; x++) {
                ...
                water_amount =
                        StrictMath.min(StrictMath.min(water.getPixel(x, y), h - avr_height), min_d);
                ...
                if (d1 > 0) {
                    ...
                    sediment_d.putPixel(
                            x, y + 1, sediment_d.getPixel(x, y + 1) + sediment_transport);
                }
                if (d2 > 0) {
                    ...
                    sediment_d.putPixel(
                            x - 1, y, sediment_d.getPixel(x - 1, y) + sediment_transport);
                }
                if (d3 > 0) {
                ...
                if (d5 > 0) {
                    ...
                    water_d.putPixel(
                            x - 1, y + 1, water_d.getPixel(x - 1, y + 1) + water_transport);
                    sediment_transport = d5 * sediment_per_height;
                    sediment_d.putPixel(
                            x - 1, y + 1, sediment_d.getPixel(x - 1, y + 1) + sediment_transport);
                }
                if (d6 > 0) {
    ...
    private static final void sedimentation() {
    ...
    private static final void score(int i) {
    ...
    private static final void timer(int i, long start) {
        long stop = System.currentTimeMillis();
        System.out.println((stop - start) / 1000f);
    }

    private static final void diff(int i) {
        float diff = old.channelDifference(height).sum();
        System.out.println(diff / (old.width * old.height));
        old = height.copy();
    }

    public static final Channel erodeFast(Channel channel, int iterations) {
        height = channel;
