        ...
        for (int i = 0; i < maxpoints; i++) {
            ...
            for (int t = 0; t < transformations.length; t++) {
                if (p < probabilities[t]) {
                    x_coord =
                            (transformations[t][2][0] - transformations[t][3][0]) * x_coord
                                    + (transformations[t][2][0] - transformations[t][1][0])
                                            * y_coord
                                    + transformations[t][0][0];
                    y_coord =
                            (transformations[t][2][1] - transformations[t][3][1]) * x_temp
                                    + (transformations[t][2][1] - transformations[t][1][1])
                                            * y_coord
                                    + transformations[t][0][1];
                    break;
                }
            }
