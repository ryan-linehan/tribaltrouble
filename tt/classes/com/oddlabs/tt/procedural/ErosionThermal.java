...
public final strictfp class ErosionThermal {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            for (int y = 1; y < channel.height - 1; y++) {
                for (int x = 1; x < channel.width - 1; x++) {
                    ...
                    if (d1 > 0)
                        height_d.putPixel(x, y + 1, height_d.getPixel(x, y + 1) + factor * d1);
                    if (d2 > 0)
                        height_d.putPixel(x - 1, y, height_d.getPixel(x - 1, y) + factor * d2);
                    if (d3 > 0)
                        height_d.putPixel(x + 1, y, height_d.getPixel(x + 1, y) + factor * d3);
                    if (d4 > 0)
                        height_d.putPixel(x, y - 1, height_d.getPixel(x, y - 1) + factor * d4);
                    if (d5 > 0)
                        height_d.putPixel(
                                x - 1, y + 1, height_d.getPixel(x - 1, y + 1) + factor * d5);
                    if (d6 > 0)
                        height_d.putPixel(
                                x + 1, y + 1, height_d.getPixel(x + 1, y + 1) + factor * d6);
                    if (d7 > 0)
                        height_d.putPixel(
                                x - 1, y - 1, height_d.getPixel(x - 1, y - 1) + factor * d7);
                    if (d8 > 0)
                        height_d.putPixel(
                                x + 1, y - 1, height_d.getPixel(x + 1, y - 1) + factor * d8);
                }
            }
    ...
    public static final Channel erodeFast2(Channel channel, int iterations) {
        height = channel;
    ...
    public static final Channel erodeFast3(Channel channel, int iterations) {
        height = channel;
