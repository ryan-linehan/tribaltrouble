...
public final strictfp class Grass {
    ...
    public Grass(int width, int height, int leaves, int seed, int clusters, int smooth) {
        ...
        for (int leaf = 0; leaf < leaves; leaf++) {
            ...
            float p1_x =
                    (float) cluster / clusters
                            + Tools.gaussify(Tools.gaussify(random.nextFloat())) / clusters;
            ...
            float p2_x =
                    StrictMath.max(
                            ...
                            StrictMath.min(
                                    p1_x
                                            + aspect * Tools.gaussify(random.nextFloat())
                                            - 0.5f * aspect,
                                    0.975f));
            ...
            if (p2_y < 0.2 && random.nextFloat() < 0.5) {
                p3_y =
                        StrictMath.max(
                                0.025f, StrictMath.min(p2_y * (random.nextFloat() + 1f), 0.975f));
            } else {
                p3_y =
                        StrictMath.max(
                                0.025f,
                                StrictMath.min(
                                        p2_y
                                                + 1.25f * p2_y * Tools.gaussify(random.nextFloat())
                                                - 0.5f * p2_y,
                                        0.975f));
            }
            p2_x = (p1_x + p3_x) / 2f;
            ...
            float l1 =
                    (float)
                            StrictMath.sqrt(
                                    (p2_x - p1_x) * (p2_x - p1_x) + (p2_y - p1_y) * (p2_y - p1_y));
            float l2 =
                    (float)
                            StrictMath.sqrt(
                                    (p3_x - p2_x) * (p3_x - p2_x) + (p3_y - p2_y) * (p3_y - p2_y));
                ...
                if (twist < 0.5) {
                    ...
                    rd =
                            (width >> 8)
                                    * radius
                                    * ((float)
                                            (StrictMath.cos(3 * StrictMath.PI * t)
                                                    + 1.5f
                                                    + 0.5 * (1f - t)))
                                    * 0.5f;
                }
                            ...
                            if (smooth == 1) {
                                ...
                                if (t < fadein) {
                                    alpha =
                                            StrictMath.min(
                                                    alpha,
                                                    Tools.interpolateLinear(0f, 1f, t / fadein));
                                }
