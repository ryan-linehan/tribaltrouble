...
public final strictfp class Perlin {
    ...
    private final void mergeNoiseChannels(
            int width, int height, float pers, int oct, int interpolation_method) {
        ...
        if (interpolation_method == SMOOTH) {
            ...
            while (width >> 3 > (int) StrictMath.pow(2, method_threshold)) method_threshold++;
        }
            ...
            if (width == octave.width
                ...
                for (y_pixel = 0; y_pixel < height; y_pixel++) {
                    for (x_pixel = 0; x_pixel < width; x_pixel++) {
                        channel.putPixel(
                                x_pixel,
                                y_pixel,
                                channel.getPixel(x_pixel, y_pixel)
                                        + octave.getPixel(x_pixel, y_pixel) * amplitude);
                    }
                }
            } else if (interpolation_method != CUBIC
                    && i >= method_threshold) { // interpolate linear
                ...
                for (y_block_lo = 0; y_block_lo < y_blocks; y_block_lo++) {
                    ...
                    for (x_block_lo = 0; x_block_lo < x_blocks; x_block_lo++) {
                        ...
                        y_incr1 =
                                (octave.getPixel(x_block_lo, y_block_hi)
                                                - octave.getPixel(x_block_lo, y_block_lo))
                                        / block_height;
                        y_incr2 =
                                (octave.getPixel(x_block_hi, y_block_hi)
                                                - octave.getPixel(x_block_hi, y_block_lo))
                                        / block_height;
                        ...
                        for (int y = 0; y < block_height; y++) {
                            ...
                            for (int x = 0; x < block_width; x++) {
                                ...
                                channel.putPixel(
                                        x_pixel,
                                        y_pixel,
                                        channel.getPixel(x_pixel, y_pixel) + val * amplitude);
                            }
                        }
                    }
                }
                    ...
                    for (int y = 0; y < block_height; y++) {
                        ...
                        for (x_block_lo = 0; x_block_lo < x_blocks; x_block_lo++) {
                            ...
                            for (int x = 0; x < block_width; x++) {
                                ...
                                channel.putPixelWrap(
                                        x_pixel,
                                        y_pixel,
                                        channel.getPixelWrap(x_pixel, y_pixel) + val * amplitude);
                            }
                        }
                    }
                }
                ...
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        channel.putPixel(
                                x,
                                y,
                                (float) StrictMath.sin(StrictMath.PI * 8 * channel.getPixel(x, y)));
                    }
                }
                break;
            case XSINE:
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        channel.putPixel(
                                ...
                                (float)
                                        StrictMath.sin(
                                                x / ((width / 256f) * StrictMath.PI)
                                                        + channel.getPixel(x, y)));
                    }
                }
                break;
            case NORMAL_MOD:
