...
public final strictfp class GeneratorIron extends TextureGenerator {
    ...
    public final Texture[] generate() {
        ...
        Channel rock_bump =
                new Voronoi(TEXTURE_SIZE, 8, 8, 1, 1f, seed)
                        .getDistance(-1f, 0f, 0f)
                        .multiply(.49f);
        rock_bump.channelAdd(
                new Voronoi(TEXTURE_SIZE, 8, 8, 1, 1f, seed + 1)
                        .getDistance(-1f, 0f, 0f)
                        .multiply(.49f));
        ...
        Layer rust =
                new Layer(
                        new Channel(TEXTURE_SIZE, TEXTURE_SIZE).fill(.4f),
                        new Channel(TEXTURE_SIZE, TEXTURE_SIZE).fill(.15f),
                        new Channel(TEXTURE_SIZE, TEXTURE_SIZE).fill(0f),
                        noise2.copy().gamma2().invert());
        rock.layerBlend(rust);
        rock.bump(rock_bump, TEXTURE_SIZE / 256f, 0f, 1f, 1f, 1f, 1f, 0f, 0f, 0f)
                .gamma(.5f)
                .dynamicRange(0f, .75f);
        rock.bumpSpecular(rock_bump, 1f, 0f, 1f, 0f, .1f, .1f, .1f, 1);
        Layer stain =
                new Layer(
                        noise2.copy().multiply(.4f),
                        noise2.copy().multiply(.15f),
                        noise2.copy().multiply(0f),
                        noise2.copy().gamma(.75f).rotate(90));
        ...
        textures[0] =
                new Texture(
                        new GLIntImage(rock).createMipMaps(),
                        GL11.GL_RGB,
