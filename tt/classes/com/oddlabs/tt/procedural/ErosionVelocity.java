...
public final strictfp class ErosionVelocity {
        ...
        for (int iter = 1; iter <= iterations; iter++) {
            ...
            for (int y = 0; y < channel.height; y++) {
                for (int x = 0; x < channel.width; x++) {
                    ...
                    while (i >= 0 && i < channel.width && j >= 0 && j < channel.height) {
                        ...
                        if (h1 >= h && h2 >= h && h3 >= h && h4 >= h && h5 >= h && h6 >= h
                                && h7 >= h && h8 >= h) {
                            w.putPixelSafe(
                                    i,
                                    j,
                                    w.getPixelSafe(i, j) + water_particle); // add water to flowmap
                            break;
                        }
                        ...
                        w.putPixelSafe(
                                i,
                                j,
                                w.getPixelSafe(i, j) + water_particle); // add water to water map
                        v.putPixelSafe(
                                i, j, v.getPixelSafe(i, j) + max); // add velocity to velocity map
            ...
            for (int y = 0; y < channel.height; y++) {
                for (int x = 0; x < channel.width; x++) {
                    ...
                    channel.putPixel(
                            x, y, channel.getPixel(x, y) - erosion_factor * v.getPixel(x, y));
                }
            }
    ...
    public final Channel velocityFieldErosion2(Channel channel, int iterations) {
        ...
        Channel v = new Channel(channel.width, channel.height); // velocity
        int[][][] dir =
                new int[channel.width][channel.height][2]; // velocity direction (end coords)
                ...
                for (int x = 0; x < channel.width; x++) {
                    ...
                    while (i >= 0 && i < channel.width && j >= 0 && j < channel.height) {
                        ...
                        if (dir[i][j][0] == -2) {
                            ...
                            max =
                                    channel.getPixel(i, j)
                                            - channel.getPixelSafe(dir[i][j][0], dir[i][j][1]);
                            k = dir[i][j][0];
                            l = dir[i][j][1];
                        }
                ...
                for (int y = 0; y < channel.height; y++) {
                    for (int x = 0; x < channel.width; x++) {
                        ...
                        s.putPixelSafe(
                                dir[x][y][0],
                                dir[x][y][1],
                                s.getPixelSafe(dir[x][y][0], dir[x][y][1]) + ds);
                    }
                }
            }
                ...
                for (int x = 0; x < channel.width; x++) {
                    ...
                    while (true /*i >= 0 && i < channel.width && j >= 0 && j < channel.height*/) {
                        ...
                        if (dir[i][j][0] == -2) {
                            ...
                            max =
                                    channel.getPixelWrap(i, j)
                                            - channel.getPixelWrap(dir[i][j][0], dir[i][j][1]);
                            k = dir[i][j][0];
                            l = dir[i][j][1];
                        }
                ...
                for (int y = 0; y < channel.height; y++) {
                    for (int x = 0; x < channel.width; x++) {
                        ...
                        s.putPixelWrap(
                                dir[x][y][0],
                                dir[x][y][1],
                                s.getPixelWrap(dir[x][y][0], dir[x][y][1]) + ds);
                    }
                }
            }
