...
abstract strictfp class ShadowRenderer {
    private static ShortBuffer shadow_indices =
            BufferUtils.createShortBuffer(
                    HeightMap.GRID_UNITS_PER_PATCH * HeightMap.GRID_UNITS_PER_PATCH * 2 * 3);
    ...
    protected final void renderShadow(
            LandscapeRenderer renderer, float shadow_size, float f_x, float f_y) {
        assert shadow_size >= 0;
        // Redundant glLoadIdentity on the texture matrix stack to work around GL_OBJECT_PLANE not
        // always updating
        GL11.glLoadIdentity();
        ...
        int grid_end_x =
                StrictMath.min(
                        max_grid_index, (int) ((f_x + half_shadow_size) / meters_per_grid_unit));
        int grid_end_y =
                StrictMath.min(
                        max_grid_index, (int) ((f_y + half_shadow_size) / meters_per_grid_unit));
        ...
        for (int patch_y = patch_start_y; patch_y <= patch_end_y; patch_y++) {
            for (int patch_x = patch_start_x; patch_x <= patch_end_x; patch_x++) {
                int local_start_x =
                        StrictMath.max(grid_start_x, patch_x * grid_units_per_patch)
                                & (grid_units_per_patch - 1);
                int local_start_y =
                        StrictMath.max(grid_start_y, patch_y * grid_units_per_patch)
                                & (grid_units_per_patch - 1);
                int local_end_x =
                        StrictMath.min(grid_end_x, (patch_x + 1) * grid_units_per_patch - 1)
                                & (grid_units_per_patch - 1);
                int local_end_y =
                        StrictMath.min(grid_end_y, (patch_y + 1) * grid_units_per_patch - 1)
                                & (grid_units_per_patch - 1);
                renderer.renderShadow(
                        patch_x, patch_y, local_start_x, local_start_y, local_end_x, local_end_y);
