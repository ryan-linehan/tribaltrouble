...
final strictfp class Sprite {
    public static final int LOWDETAIL_NORMAL = 1;
    public static final int LOWDETAIL_TEAMDECAL = 2;
    private static final int TEXTURE_NORMAL = 0;
    private static final int TEXTURE_TEAM = 1;
    private static final float[] respond_color = new float[] {1f, 1f, 1f, 1f};
    ...
    public Sprite(
        ...
        expandAnimation(
                ...
                tmp_vertices,
                tmp_normals,
                sprite_info.getVertices(),
                sprite_info.getNormals(),
                sprite_info.getSkinNames(),
                sprite_info.getSkinWeights(),
                bounds);
        ...
        FloatBuffer temp_vertices_and_normals =
                BufferUtils.createFloatBuffer(vert_and_normal_buffer_size);
        ...
        int memory_size =
                tmp_indices.length * 2 + (tmp_texcoords.length + vert_and_normal_buffer_size) * 4;
        global_size += memory_size;
        //		System.out.println("sprite size: " + memory_size);
        int color_format = Globals.COMPRESSED_RGB_FORMAT;
        if (alpha) color_format = Globals.COMPRESSED_RGBA_FORMAT;
        ...
        for (int i = 0; i < texture_names.length; i++) {
            textures[i][TEXTURE_NORMAL] =
                    getTextureForName(texture_names[i][0], color_format, mipmap_cutoff, max_alpha);
            if (texture_names[i][TEXTURE_TEAM] != null)
                textures[i][TEXTURE_TEAM] =
                        getTextureForName(
                                texture_names[i][1],
                                Globals.COMPRESSED_RGB_FORMAT,
                                mipmap_cutoff,
                                max_alpha);
            else textures[i][TEXTURE_TEAM] = null;
        }
    ...
    private final void expandAnimation(
            ...
            float[][][] tmp_vertices,
            float[][][] tmp_normals,
            float[] initial_pose_vertices,
            float[] initial_pose_normals,
            byte[][] skin_names,
            float[][] skin_weights,
            BoundingBox[] bounding_boxes) {
        int num_bones = animations[0].getFrames()[0].length / 12;
        Matrix4f[] frame_bones = new Matrix4f[num_bones];
        for (int bone = 0; bone < frame_bones.length; bone++) frame_bones[bone] = new Matrix4f();
        ...
        for (int anim = 0; anim < animations.length; anim++) {
            ...
            for (int frame = 0; frame < num_frames; frame++) {
                ...
                for (int vertex = 0; vertex < num_vertices; vertex++) {
                    ...
                    // Use Math.sqrt here for efficiency. Only used for normals (not gamestate
                    // affecting) anyway.
                    float vec_len_inv =
                            1f
                                    / (float)
    ...
    private static final Texture getTextureForName(
            String texture_name, int color_format, int mipmap_cutoff, boolean max_alpha) {
    ...
    public final void setupWithColor(
            FloatBuffer color, int tex_index, boolean respond, boolean modulate_color) {
    ...
    private final void doSetup(
        ...
        if (!modulate_color && (hasTeamDecal() || respond)) {
            ...
            if (respond) {
                ...
                GL11.glBindTexture(
                        GL11.GL_TEXTURE_2D, textures[tex_index][TEXTURE_TEAM].getHandle());
            }
    ...
    private final int getFrameCapped(int animation, int frame) {
        ...
        if (type_array[animation] == AnimationInfo.ANIM_LOOP) return frame % anim_length;
        else return StrictMath.min(frame, anim_length - 1);
    }
