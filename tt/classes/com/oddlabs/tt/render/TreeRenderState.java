...
final strictfp class TreeRenderState implements LODObject {
    ...
    public final int getTriangleCount(int index) {
        ...
        if (index == SpriteRenderer.HIGH_POLY)
            return tree.getTrunk().getSprite(0).getTriangleCount()
                    + tree.getCrown().getSprite(0).getTriangleCount();
    ...
    public final float getEyeDistanceSquared() {
        ...
        return RenderTools.getEyeDistanceSquared(
                tree_supply, camera.getCurrentX(), camera.getCurrentY(), camera.getCurrentZ());
    }
}
