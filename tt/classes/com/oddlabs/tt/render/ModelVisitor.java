...
abstract strictfp class ModelVisitor {
    ...
    public void markDetailPolygon(ElementRenderState render_state, int index) {
        Model model = render_state.model;
        render_state
                .getRenderer(model.getSpriteRenderer())
                .addToRenderList(
                        index, render_state, render_state.render_state.isResponding(model));
    }
    ...
    public final float getEyeDistanceSquared(ElementRenderState render_state) {
        ...
        return RenderTools.getEyeDistanceSquared(
                model, camera.getCurrentX(), camera.getCurrentY(), camera.getCurrentZ());
    }
