...
strictfp class SelectableVisitor extends ModelVisitor {
    ...
    private static final float[] COLOR_RED_HOVER = {.7f, 0f, 0f};
    private static final float[] COLOR_GREEN = {0f, 1f, 0f};
    private static final float[] COLOR_GREEN_HOVER = {0f, .7f, 0f};
    private static final float[] COLOR_BLUE = {0f, 0f, 1f};
    private static final float[] COLOR_BLUE_HOVER = {0f, 0f, .7f};
    ...
    public final float[] getSelectionColor(ElementRenderState render_state) {
        ...
        if (render_state.render_state.isSelected(model)) {
            if (model.getOwner() == local_player) return COLOR_GREEN;
            else if (local_player.isEnemy(model.getOwner())) return COLOR_RED;
            else return COLOR_BLUE;
        } else if (render_state.render_state.isHovered(model)) {
            if (model.getOwner() == local_player) return COLOR_GREEN_HOVER;
            else if (local_player.isEnemy(model.getOwner())) return COLOR_RED_HOVER;
            else return COLOR_BLUE_HOVER;
        } else return model.getOwner().getColor();
    }

    public final void markDetailPoint(ElementRenderState render_state) {
        Selectable selectable = (Selectable) render_state.model;
        if (!selectable.isDead()) super.markDetailPoint(render_state);
    }
}
