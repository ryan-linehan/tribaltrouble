...
final strictfp class RenderTools {
    ...
    static final int NOT_IN_FRUSTUM = 1;
    static final int IN_FRUSTUM = 2;
    static final int ALL_IN_FRUSTUM = 3;
    ...
    static int inFrustum(BoundingBox box, float[][] frustum) {
        ...
        for (int f = 0; f < 6; f++) {
            ...
            if (frustum[f][0] * box.bmin_x
                            + frustum[f][1] * box.bmin_y
                            + frustum[f][2] * box.bmin_z
                            + frustum[f][3]
                    > 0) {
                if (!all_in) continue;
                else one_in = true;
            ...
            if (!one_in) return NOT_IN_FRUSTUM;
        }
        if (all_in) return ALL_IN_FRUSTUM;
        else return IN_FRUSTUM;
    }

    static float getEyeDistanceSquared(
            BoundingBox box, float camera_x, float camera_y, float camera_z) {
    ...
    static float getCameraDistanceSquared(
            BoundingBox box, float camera_x, float camera_y, float camera_z) {
        float distz = camera_z - box.getCZ();
    ...
    static void draw(BoundingBox box, float r, float g, float b) {
        DebugRender.drawBox(
                box.bmin_x, box.bmax_x, box.bmin_y, box.bmax_y, box.bmin_z, box.bmax_z, r, g, b);
    }

    static void draw(BoundingBox box, int bound_type, float r, float g, float b) {
        if (Globals.isBoundsEnabled(bound_type)) draw(box, r, g, b);
    }
}
