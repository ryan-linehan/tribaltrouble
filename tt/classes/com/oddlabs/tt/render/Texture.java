...
public final strictfp class Texture extends NativeResource {
    ...
    private static final IntBuffer size_buffer;
    private static final FloatBuffer border_color_buffer;
    ...
    private static final int bestWrap(int wrap) {
        if (wrap == GL12.GL_CLAMP_TO_EDGE && !GL.getCapabilities().OpenGL12) {
            return GL11.GL_CLAMP;
        } else return wrap;
    }

    private static final int initTexture(
            int min_filter, int mag_filter, int wrap_s, int wrap_t, int max_mipmap_level) {
        ...
        GL11.glTexParameterfv(
                GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_BORDER_COLOR, border_color_buffer);
        //		GL11.glTexParameterf(GL11.GL_TEXTURE_2D,
        // org.lwjgl.opengl.EXTTextureFilterAnisotropic.GL_TEXTURE_MAX_ANISOTROPY_EXT, 10f);
        return tex_handle;
    }
    ...
    public Texture(TextureFile texture_file) {
        ...
        if (texture_file.isDXTImage()) {
            ...
            GLImage[] mipmaps;
            if (texture_file.getMinFilter() == GL11.GL_LINEAR_MIPMAP_NEAREST
                    || texture_file.getMinFilter() == GL11.GL_LINEAR_MIPMAP_LINEAR) {
            ...
            } else mipmaps = new GLImage[] {img};
            uploadTexture(
                    mipmaps, texture_file.getInternalFormat(), texture_file.getMaxMipmapLevel());
        }
    }
    ...
    private static final int getDetailShift(int num_mipmaps) {
        return StrictMath.min(
                num_mipmaps - 1, Globals.TEXTURE_MIP_SHIFT[Settings.getSettings().graphic_detail]);
    }

    private static final int getMaxMipmapIndex(
            int num_mipmaps, int max_mipmap_level, int detail_shift) {
    ...
    private final void uploadDXTTexture(DXTImage dxt_image, TextureFile texture_file) {
        ...
        int max_index =
                getMaxMipmapIndex(
                        dxt_image.getNumMipMaps(), texture_file.getMaxMipmapLevel(), detail_shift);
        int total_size = 0;
        for (int i = 0; i < max_index; i++) {
            ...
            if (GL.getCapabilities().GL_EXT_texture_compression_s3tc) {
                ...
                GLState.glCompressedTexImage2D(
                        GL11.GL_TEXTURE_2D,
                        i,
                        dxt_image.getInternalFormat(),
                        dxt_image.getWidth(mipmap_level),
                        dxt_image.getHeight(mipmap_level),
                        0,
                        dxt_image.getMipMap());
            } else {
    ...
    private final void uploadTexture(GLImage[] mipmaps, int internal_format, int max_mipmap_level) {
        ...
        for (int i = 0; i < max_index; i++) {
            ...
            assert Utils.isPowerOf2(mipmap.getWidth()) && Utils.isPowerOf2(mipmap.getHeight())
                    : "Mipmap level " + i + " dimensions are not power of two";
            GL11.glTexImage2D(
                    GL11.GL_TEXTURE_2D,
                    i,
                    internal_format,
        ...
        for (int i = 0; i < max_index; i++) {
            GLImage mipmap = mipmaps[i + detail_shift];
            int size =
                    determineMipMapSize(i, internal_format, mipmap.getWidth(), mipmap.getHeight());
            total_size += size;
        }
        setSize(total_size);
    }
    ...
    private static int determineMipMapSize(int mipmap, int internal_format, int width, int height) {
        ...
        if (Settings.getSettings().useTextureCompression()) {
            GL11.glGetTexLevelParameteriv(
                    GL11.GL_TEXTURE_2D, mipmap, GL13.GL_TEXTURE_COMPRESSED, size_buffer);
            compressed = size_buffer.get(0) == GL11.GL_TRUE;
        }
        if (compressed) {
            GL11.glGetTexLevelParameteriv(
                    GL11.GL_TEXTURE_2D, mipmap, GL13.GL_TEXTURE_COMPRESSED_IMAGE_SIZE, size_buffer);
