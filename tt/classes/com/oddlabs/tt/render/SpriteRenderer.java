...
public final strictfp class SpriteRenderer {
    public static final int HIGH_POLY = 0;
    public static final int LOW_POLY = 1;
    ...
    public final void setupWithColor(
            int index, FloatBuffer material_color, boolean respond, boolean modulate_tex1) {
        getSpriteList()
                .getSprite(index)
                .setupWithColor(material_color, tex_index, respond, modulate_tex1);
    }
    ...
    public final void renderAll() {
        ...
        for (int i = 0; i < no_detail_render_list.size(); i++) {
            ModelState model = (ModelState) no_detail_render_list.get(i);
            no_detail_render_list.set(i, null);
            if (Globals.draw_misc) renderNoDetail(model);
        }
        finishNoDetail();
        clearRenderLists();
    }
