...
public abstract strictfp class BillboardPainter {
    ...
    private static final void initClipPlane(
            int clip_enum,
            int face_index,
            int vertex_index1,
            int vertex_index2,
            short[] indices,
            float[] face_tex_coords,
            float handedness) {
    ...
    private static final float getElement(
            int face_index,
            int vertex_index,
            int element_index,
            int vertex_size,
    ...
    public static final void loadFaceMatrixAndClipPlanes(
            int face_index, short[] indices, float[] face_vertices, float[] face_tex_coords) {
        // Find object space to texture space matrix, mapping vectors in object space to vectors in
        // texture space
        Vector3f v1 =
                new Vector3f(
                        getElement(face_index, 0, 0, 3, indices, face_vertices),
                        getElement(face_index, 0, 1, 3, indices, face_vertices),
                        getElement(face_index, 0, 2, 3, indices, face_vertices));
        Vector3f v2 =
                new Vector3f(
                        getElement(face_index, 1, 0, 3, indices, face_vertices),
                        getElement(face_index, 1, 1, 3, indices, face_vertices),
                        getElement(face_index, 1, 2, 3, indices, face_vertices));
        Vector3f v3 =
                new Vector3f(
                        getElement(face_index, 2, 0, 3, indices, face_vertices),
                        getElement(face_index, 2, 1, 3, indices, face_vertices),
                        getElement(face_index, 2, 2, 3, indices, face_vertices));
        Vector2f w1 =
                new Vector2f(
                        getElement(face_index, 0, 0, 2, indices, face_tex_coords),
                        getElement(face_index, 0, 1, 2, indices, face_tex_coords));
        Vector2f w2 =
                new Vector2f(
                        getElement(face_index, 1, 0, 2, indices, face_tex_coords),
                        getElement(face_index, 1, 1, 2, indices, face_tex_coords));
        Vector2f w3 =
                new Vector2f(
                        getElement(face_index, 2, 0, 2, indices, face_tex_coords),
        ...
        obj_to_tex_matrix.m00 = tangent3.x;
        obj_to_tex_matrix.m01 = tangent3.y;
        obj_to_tex_matrix.m02 = tangent3.z;
        obj_to_tex_matrix.m03 = 0;
        obj_to_tex_matrix.m10 = cross.x;
        obj_to_tex_matrix.m11 = cross.y;
        obj_to_tex_matrix.m12 = cross.z;
        obj_to_tex_matrix.m13 = 0;
        obj_to_tex_matrix.m20 = n.x;
        obj_to_tex_matrix.m21 = n.y;
        obj_to_tex_matrix.m22 = n.z;
        obj_to_tex_matrix.m23 = 0;
        obj_to_tex_matrix.m30 = 0;
        obj_to_tex_matrix.m31 = 0;
        obj_to_tex_matrix.m32 = 0;
        obj_to_tex_matrix.m33 = 1;
        ...
        // Combine matrices resulting in the matrix converting points in texture space to points in
        // object space
        ...
        // Invert the matrix to get the matrix from points in object space to points in texture
        // space
        result.invert();
