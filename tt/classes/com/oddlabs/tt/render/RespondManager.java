...
public final strictfp class RespondManager implements Animated {
    ...
    private void timeout() {
        ...
        while (!respond_timeouts.isEmpty()
                && (head_timeout = (Timeout) respond_timeouts.firstKey()).timeout <= time) {
    ...
    private void removeResponder(Object target) {
        ...
        if (timeout != null) {
            ...
            if (timeout.stop_action != null) timeout.stop_action.run();
        }
    }
    ...
    final boolean isResponding(Object target) {
        ...
        else return isResponding((Timeout) respond_targets.get(target));
    }
    ...
    private boolean isResponding(Timeout timeout) {
        ...
        return time_diff > 0
                && (time_diff >= SECONDS_PER_PICK_RESPOND - blink || time_diff <= blink);
    }

    public final void updateChecksum(StateChecksum checksum) {}

    private static final strictfp class Timeout implements Comparable {
        ...
        public final int compareTo(Object other) {
            ...
            if (diff != 0f) return (int) diff;
            else return id - timeout_obj.id;
        }
    }
}
