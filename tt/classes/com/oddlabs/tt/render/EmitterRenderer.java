...
final strictfp class EmitterRenderer {
    private static final float SQRT_2 = (float) StrictMath.sqrt(2f);
    private static final float ROTATION_FACTOR = 60f;

    private static final StrictVector3f right_plus_up = new StrictVector3f();
    private static final StrictVector3f right_minus_up = new StrictVector3f();
    private static final FloatBuffer color_buffer = BufferUtils.createFloatBuffer(4);

    private static final StrictMatrix4f view_matrix = new StrictMatrix4f();
    ...
    public static void render(RenderQueues render_queues, List emitter_queue, CameraState state) {
        ...
        float upx = tmp_camera.getModelView().m01;
        float upy = tmp_camera.getModelView().m11;
        float upz = tmp_camera.getModelView().m21;
    ...
    private static void render2DParticle(Particle particle, Emitter emitter) {
        ...
        GL11.glVertex3f(
                x - right_plus_up.getX() * radius_x,
                y - right_plus_up.getY() * radius_y,
                z - right_plus_up.getZ() * radius_z);
        GL11.glTexCoord2f(particle.getU2(), particle.getV2());
        GL11.glVertex3f(
                x + right_minus_up.getX() * radius_x,
                y + right_minus_up.getY() * radius_y,
                z + right_minus_up.getZ() * radius_z);
    ...
    private static void render(RenderQueues render_queues, Emitter emitter) {
        ...
        if (textures != null) {
            ...
            for (int j = 0; j < particles.length; j++) {
                GL11.glBindTexture(
                        GL11.GL_TEXTURE_2D, render_queues.getTexture(textures[j]).getHandle());
            ...
            for (int j = 0; j < particles.length; j++) {
                for (int i = particles[j].size() - 1; i >= 0; i--) {
                    ...
                    SpriteRenderer sprite_renderer =
                            render_queues.getRenderer(sprite_renderers[index]);
