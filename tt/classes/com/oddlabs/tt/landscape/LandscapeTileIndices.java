...
public final strictfp class LandscapeTileIndices {
    ...
    private static int getPlaneDirection(int quad_size, float x, float y) {
        if (x + y > quad_size) {
            if (x > y) return LandscapeTileTriangle.EAST_EXP;
            else return LandscapeTileTriangle.NORTH_EXP;
        } else {
            if (x > y) return LandscapeTileTriangle.SOUTH_EXP;
            else return LandscapeTileTriangle.WEST_EXP;
        }
    }
    ...
    public LandscapeTileIndices(HeightMap heightmap, int patch_exp) {
        ...
        LandscapeTileTriangle lower =
                new LandscapeTileTriangle(
                        patch_exp, 0, LandscapeTileTriangle.SOUTH_WEST, sw, se, nw);
        LandscapeTileTriangle higher =
                new LandscapeTileTriangle(
                        patch_exp, 0, LandscapeTileTriangle.NORTH_EAST, ne, nw, se);
    ...
    public final void fillCoverIndices(
            ShortBuffer buffer,
            int lod,
            int border_set,
            int start_x,
    ...
    float getHeight(int lod, float x, float y) {
        ...
        LandscapeTileTriangle[] quad =
                quad_to_planes[lod][
                        getQuadIndex(patch_exp, num_quads_exp, patch_relative_x, patch_relative_y)];
        ...
        for (int i = 0; i < errors.length; i++) {
            ...
            for (int y = 0; y < patch_size - 1; y++) {
                ...
                for (int x = 0; x < patch_size - 1; x++) {
                    ...
                    LandscapeTileTriangle plane =
                            current_quads[getQuadIndex(patch_exp, num_quads_exp, x, y)][
                                    plane_direction];
                    ...
                    if (actual_height < heightmap.getSeaLevelMeters()) below = true;
                    else if (actual_height > heightmap.getSeaLevelMeters()) above = true;
                }
            }
            errors[i] = max_error;
        }
