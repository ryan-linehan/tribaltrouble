...
public abstract strictfp class AbstractPatchGroup extends BoundingBox {
    ...
    protected AbstractPatchGroup(
            HeightMap heightmap, float patch_size, int x, int y, AbstractPatchGroup parent) {
        this.parent = parent;
        patch_radius =
                (float) StrictMath.sqrt(2) * patch_size * heightmap.getMetersPerPatch() * 0.5f;
    ...
    final void editHeight(float height) {
        checkBoundsZ(height);
        if (parent != null) parent.editHeight(height);
    }
