...
public final strictfp class LandscapeTileTriangle {
    ...
    private static int getDirection(int patch_exp, int i0, int i1) {
        ...
        if (i0_x == 0 && i1_x == 0) return WEST;
        else if (i0_x == patch_size - 1 && i1_x == patch_size - 1) return EAST;
        else if (i0_y == 0 && i1_y == 0) return SOUTH;
        else if (i0_y == patch_size - 1 && i1_y == patch_size - 1) return NORTH;
        else return INNER;
    }
    ...
    private void mapIndex(LandscapeTileTriangle[] quad_planes, int check_direction_exp) {
        if ((direction & (1 << check_direction_exp)) != 0) quad_planes[check_direction_exp] = this;
    }
    ...
    public final void putIndices(int lod, int border_set, ShortBuffer indices) {
        // System.out.println("lod = " + lod + " | Integer.toHexString(border_set) = " +
        // Integer.toHexString(border_set) + " | Integer.toHexString(border_direction) = " +
        // Integer.toHexString(border_direction));
