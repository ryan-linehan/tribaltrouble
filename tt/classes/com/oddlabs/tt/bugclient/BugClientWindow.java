...
public final strictfp class BugClientWindow {
    ...
    private static void showReporter(
            final Runnable done_action, final URL url, final int revision, final File log_dir) {
        try {
                        ...
                        String cancel_text =
                                Utils.getBundleString(
                                        ResourceBundle.getBundle(
                                                com.oddlabs.tt.gui.CancelButton.class.getName()),
                                        "cancel");
        ...
        frame.getRootPane()
                ...
                .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), cancel_action);
        ...
        JTextArea description =
                createReadOnlyText(
                        Utils.getBundleString(bundle, "describe")
                                + "\n"
                                + Utils.getBundleString(bundle, "note"));

        // Email field
        JLabel email_label = new JLabel(Utils.getBundleString(bundle, "email"));
        final JFormattedTextField email_text =
                new JFormattedTextField(new RegexFormatter(com.oddlabs.util.Utils.EMAIL_PATTERN));
                ...
                new AbstractAction() {
                    {
                        String ok_text =
                                Utils.getBundleString(
                                        ResourceBundle.getBundle(
                                                com.oddlabs.tt.gui.OKButton.class.getName()),
                                        "ok");
                    ...
                    public final void actionPerformed(ActionEvent e) {
                        ...
                        uploadBugReport(
                                ...
                                url,
                                revision,
                                email_value != null ? email_value.toString() : "",
                                comment_area.getText(),
                                log_dir);
                    }
    ...
    private static void place(
            SpringLayout layout, Container parent, JComponent component, JComponent upper) {
    ...
    private static void uploadBugReport(
            final JFrame owner,
            final Runnable done_action,
            final Runnable failed_action,
            final URL url,
            final int revision,
            final String email,
                    ...
                    public final void finished() {
                        ...
                        if (getValue() != null) {
                            ...
                            JOptionPane.showMessageDialog(
                                    owner,
                                    Utils.getBundleString(bundle, "connection_failed")
                                            + "\n"
                                            + reason);
                            failed_action.run();
                        } else {
                            JOptionPane.showMessageDialog(
                                    owner, Utils.getBundleString(bundle, "success"));
                            done_action.run();
                        }
                    }
                };
        progress_dialog.setCancelAction(
                new Runnable() {
                    public final void run() {
                        worker.interrupt();
                        cancel_flag[0] = true;
    ...
    private static ProgressDialog createProgressMonitor(JFrame owner, String title) {
        ...
        final Runnable composed_cancel_action =
                new Runnable() {
                    public final void run() {
        ...
        upload_dialog.addWindowListener(
                new WindowAdapter() {
                    public final void windowClosed(WindowEvent e) {
                        cancel_action[0].run();
                    }
        ...
        layout.putConstraint(
                SpringLayout.NORTH, cancel_button, 5, SpringLayout.SOUTH, progress_bar);
        layout.putConstraint(
                SpringLayout.SOUTH, content_pane, 5, SpringLayout.SOUTH, cancel_button);
        upload_dialog.pack();
        Dimension owner_size = owner.getSize();
        Dimension dialog_size = upload_dialog.getSize();
        upload_dialog.setLocation(
                new Point(
                        (owner_size.width - dialog_size.width) / 2,
                        (owner_size.height - dialog_size.height) / 2));
