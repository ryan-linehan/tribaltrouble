...
public final strictfp class Tutorial {
    ...
    public Tutorial(
            WorldViewer viewer, TutorialInGameInfo tutorial_info, TutorialTrigger first_trigger) {
        this.viewer = viewer;
    ...
    final void next(final TutorialTrigger trigger) {
        timer.stop();
        TimerAnimation delay_timer =
                new TimerAnimation(
                        viewer.getAnimationManagerLocal(),
                        new Updatable() {
    ...
    private void next1(final TutorialTrigger trigger) {
        String text =
                Utils.getBundleString(
                        ResourceBundle.getBundle(TutorialTrigger.class.getName()),
                        trigger.getTextKey(),
                        trigger.getFormatArgs());
        info = new LabelBox(text, Skin.getSkin().getEditFont(), 400);
        info.setPos(
                BORDER_OFFSET, viewer.getGUIRoot().getHeight() - BORDER_OFFSET - info.getHeight());
        viewer.getGUIRoot().addChild(info);
        viewer.getWorld()
                .getAudio()
                .newAudio(
                        new AudioParameters(
                                viewer.getLocalPlayer().getRace().getBuildingNotificationAudio(),
                                0f,
                                0f,
                                0f,
                                AudioPlayer.AUDIO_RANK_NOTIFICATION,
                                AudioPlayer.AUDIO_DISTANCE_NOTIFICATION,
                                .25f,
                                1f,
                                1f,
                                false,
