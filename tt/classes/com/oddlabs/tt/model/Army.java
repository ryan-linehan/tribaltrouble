...
public strictfp class Army {
    ...
    public final Selectable[] filter(int ability_filter) {
        ...
        while (it.hasNext()) {
            ...
            if (s.getAbilities().hasAbilities(ability_filter)) count++;
        }
        ...
        while (it.hasNext()) {
            Selectable s = (Selectable) it.next();
            if (s.getAbilities().hasAbilities(ability_filter)) filtered[index++] = s;
        }
        return filtered;
    }
    ...
    public final boolean containsAbility(int ability_filter) {
        ...
        while (it.hasNext()) {
            Selectable s = (Selectable) it.next();
            if (s.getAbilities().hasAbilities(ability_filter)) return true;
        }
        return false;
    }
