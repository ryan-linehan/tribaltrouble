...
public abstract strictfp class WeaponFactory {
    ...
    private static final float computeTerrainBonus(HeightMap heightmap, Target src, Target dst) {
    ...
    public final void attack(Unit src, Selectable target, float factor) {
        ...
        float terrain_bonus =
                computeTerrainBonus(src.getOwner().getWorld().getHeightMap(), src, target);
        float difficulty_bonus = src.getOwner().getHitBonus();
        boolean hit =
                target.getOwner().getWorld().getRandom().nextFloat()
                        < factor
                                * (difficulty_bonus + terrain_bonus + hit_chance)
                                * (1 - target.getDefenseChance());
