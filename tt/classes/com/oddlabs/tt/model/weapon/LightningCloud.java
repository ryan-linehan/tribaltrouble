...
public final strictfp class LightningCloud implements Magic {
    ...
    public LightningCloud(
            ...
            float seconds_per_hit,
            float seconds_to_init,
        ...
        position.setZ(
                world.getHeightMap().getNearestHeight(position.getX(), position.getY()) + height);

        cloud =
                new ParametricEmitter(
                        world,
                        new CloudFunction(2.5f, .7f),
                        position,
                        0f,
                        0f,
                        .5f,
                        ...
                        25,
                        100f,
                        new Vector4f(.4f, .4f, .4f, .6f),
                        new Vector4f(0f, 0f, 0f, 0f),
                        new Vector3f(3f, 3f, 1f),
                        new Vector3f(0f, 0f, 0f),
                        seconds_to_live + seconds_to_init,
                        GL11.GL_SRC_ALPHA,
        ...
        bubbling_sound =
                world.getAudio()
                        .newAudio(
                                new AudioParameters(
                                        world.getRacesResources().getBubblingSound(),
                                        position.getX(),
                                        position.getY(),
                                        world.getHeightMap()
                                                .getNearestHeight(position.getX(), position.getY()),
    ...
    public final void animate(float t) {
        if (first_run) {
            cloud_sound =
                    owner.getWorld()
                            ...
                            .newAudio(
                                    new AudioParameters(
                                            owner.getWorld().getRacesResources().getCloudSound(),
                                            position.getX(),
                                            position.getY(),
                                            position.getZ(),
                                            AudioPlayer.AUDIO_RANK_MAGIC,
        ...
        if (hit_timer > seconds_per_hit) {
            if (target == null) {
                target =
                        owner.findNearestEnemy(
                                UnitGrid.toGridCoordinate(position.getX()),
                                UnitGrid.toGridCoordinate(position.getY()),
                                prev_target);
                if (target == null) {
                    target =
                            owner.findNearestEnemy(
            ...
            if (dist < meters_per_second * t) {
                ...
                owner.getWorld()
                        ...
                        .newAudio(
                                new AudioParameters(
                                        owner.getWorld().getRacesResources().getLightningSound(),
                                        x,
                                        y,
                                        z,
                                        AudioPlayer.AUDIO_RANK_MAGIC,
    ...
    private final void strike(Target target) {
        ...
        new Lightning(
                ...
                new Vector4f(0f, 0f, 0f, -1f / LIGHTNING_TIME),
                owner.getWorld().getRacesResources().getLightningTexture(),
                LIGHTNING_TIME,
