...
public strictfp class WeaponsProducer {
    ...
    private static final List build_list = new ArrayList();
    ...
    public WeaponsProducer(
            Building building,
            WorkerUnitContainer unit_container,
    ...
    public final void animate(float t) {
        for (int i = 0; i < production_containers.length; i++)
            if (production_containers[i].getNumSupplies() > 0
                    && production_containers[i].hasEnoughSupplies())
        ...
        if (build_list.size() > 0) {
            if (break_time <= 0) {
                if (building.getOwner().getWorld().getRandom().nextFloat() < BREAK_PROBABILITY) {
                    break_time =
                            building.getOwner().getWorld().getRandom().nextFloat() * MAX_BREAK_TIME;
                    emitter.stop();
                } else {
                    emitter.start();
                }
            ...
            startSound();
            float man_seconds_per_container =
                    unit_container.getNumSupplies() * t / build_list.size();
