...
public final strictfp class ElementNode extends AbstractElementNode {
    ...
    private final AbstractElementNode createChild(/*int level,*/ int size, int x, int y) {
        if (size != MIN_NODE_SIZE) return new ElementNode(this, /*level + 1, */ size, x, y);
        else return new ElementLeaf(this, /*level + 1, */ size, x, y);
    }

    protected final AbstractElementNode doInsertElement(Element model) {
        incElementCount();
        if (model.bmin_x >= getCX()) {
            if (model.bmin_y >= getCY()) return child3.insertElement(model);
            else if (model.bmax_y <= getCY()) return child1.insertElement(model);
        } else if (model.bmax_x <= getCX()) {
            if (model.bmin_y >= getCY()) return child2.insertElement(model);
            else if (model.bmax_y <= getCY()) return child0.insertElement(model);
        }
